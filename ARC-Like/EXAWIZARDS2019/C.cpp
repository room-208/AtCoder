...
int simulate(int i, int N, int Q, string &S, vector<char> &t, vector<char> &d) {
  for (int q = 0; q < Q; q++) {
    if (S[i] == t[q]) {
      if (d[q] == 'L') {
        i--;
...
int left_bound(int N, int Q, string &S, vector<char> &t, vector<char> &d) {
  int left = 0;
  int right = N;
  while (right - left > 1) {
    int mid = (right + left) / 2;
    if (simulate(mid, N, Q, S, t, d) == -1) {
      left = mid;
    } else {
      right = mid;
    }
...
int right_bound(int N, int Q, string &S, vector<char> &t, vector<char> &d) {
  int left = -1;
  int right = N - 1;
  while (right - left > 1) {
    int mid = (right + left) / 2;
...
int main() {
  ...
  int ans = 0;
  if (simulate(0, N, Q, S, t, d) == -1 && simulate(N - 1, N, Q, S, t, d) == 1) {
    int l = left_bound(N, Q, S, t, d);
    int r = right_bound(N, Q, S, t, d);
    ans = r - l - 1;
  } else if (simulate(0, N, Q, S, t, d) == -1) {
    int l = left_bound(N, Q, S, t, d);
    ans = N - (l + 1);
  } else if (simulate(N - 1, N, Q, S, t, d) == 1) {
    int r = right_bound(N, Q, S, t, d);
    ans = r;
  } else {
    ans = N;
