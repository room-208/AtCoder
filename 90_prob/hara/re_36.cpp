...
int main()
{
    ...
    vector<long long> ans(Q, -INF_ll);
    for (int i = 0; i < Q; i++)
    {
        chmax(ans[i], -x[q[i]] - y[q[i]] + z[0]);
        chmax(ans[i], -x[q[i]] + y[q[i]] + z[1]);
        chmax(ans[i], x[q[i]] - y[q[i]] + z[2]);
        chmax(ans[i], x[q[i]] + y[q[i]] + z[3]);
    }
