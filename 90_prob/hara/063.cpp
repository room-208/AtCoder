...
int main()
{
    ...
    vector<pair<int, int>> score(W);
    vector<int> score_sum(H * W + 1);
    vector<int> i_num;
    int ans = -1;
    for (int bit = 1; bit < (1 << H); bit++)
    {
        ...
        int N = (int)i_num.size();

        //i_1行目
        for (int j = 0; j < W; j++)
        {
            score[j] = make_pair(1, P[i_num[0]][j]);
        }
        ...
        score_sum.assign(H * W + 1, 0);

        for (int j = 0; j < W; j++)
        {
            int tmp = score[j].first;
            int p = score[j].second;
            score_sum[p] += tmp;
        }

        for (int p = 0; p <= H * W; p++)
        {
            chmax(ans, score_sum[p]);
        }
    }
