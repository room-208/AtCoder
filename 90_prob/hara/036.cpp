...
int main()
{
    ...
    long long p_p_max = -INF_ll;
    long long p_p_min = INF_ll;
    long long p_m_max = -INF_ll;
    long long p_m_min = INF_ll;
    for (int i = 0; i < N; i++)
    {
        chmax(p_p_max, x[i] + y[i]);
        chmin(p_p_min, x[i] + y[i]);
        chmax(p_m_max, x[i] - y[i]);
        chmin(p_m_min, x[i] - y[i]);
    }

    for (int i = 0; i < Q; i++)
    {
        long long dist = -INF_ll;
        chmax(dist, p_p_max - (x[q[i]] + y[q[i]]));
        chmax(dist, -p_p_min + (x[q[i]] + y[q[i]]));
        chmax(dist, p_m_max - (x[q[i]] - y[q[i]]));
        chmax(dist, -p_m_min + (x[q[i]] - y[q[i]]));
