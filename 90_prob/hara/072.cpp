...
// 深さ優先探索
void DFS(const Graph_int &G, int s, int p, int v, vector<bool> &seen, int &dist, int &max_dist)
{
    ...
    for (auto next_v : G[v])
    {
        ...
        dist++;
        DFS(G, s, v, next_v, seen, dist, max_dist); // 再帰的に探索
    }
...
int main()
{
    ...
    for (int i = 0; i < H; i++)
    {
        for (int j = 0; j < W; j++)
        {
            ...
            if (i <= H - 2)
            {
                ...
                {
                    int down = (i + 1) * W + j;
                    G[now].push_back(down);
                    G[down].push_back(now);
                }
            }
            ...
            if (j >= 1)
            {
                ...
                {
                    int left = i * W + j - 1;
                    G[now].push_back(left);
                    G[left].push_back(now);
                }
            }
            ...
            if (j <= W - 2)
            {
                ...
                {
                    int right = i * W + j + 1;
                    G[now].push_back(right);
                    G[right].push_back(now);
                }
            }
        }
    }
    ...
    for (int i = 0; i < H; i++)
    {
        for (int j = 0; j < W; j++)
        {
            ...
            DFS(G, v, v, v, seen, dist_tmp, max_dist_tmp);

            if (max_dist_tmp != -1)
            {
                dist[v] = max_dist_tmp + 1;
            }
        }
    }
