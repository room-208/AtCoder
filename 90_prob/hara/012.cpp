...
int main()
{
    ...
    cin >> Q;
    vector<int> t(Q);
    vector<int> r(Q, INF_int), c(Q, INF_int);
    vector<int> ra(Q, INF_int), ca(Q, INF_int), rb(Q, INF_int), cb(Q, INF_int);
    for (int i = 0; i < Q; i++)
    {
        cin >> t[i];
    ...
    for (int i = 0; i < Q; i++)
    {
        ...
        if (t[i] == 2)
        {
            if (color[ra[i]][ca[i]] == 1 && color[rb[i]][cb[i]] == 1)
            {
                int na = ra[i] * W + ca[i];
                int nb = rb[i] * W + cb[i];
                if (uf.issame(na, nb))
