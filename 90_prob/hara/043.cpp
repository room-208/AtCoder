...
//ダイクストラ法
vector<long long> Dijkstra(const Graph_Edge &G, int s_0, int s_1)
{
    int N = (int)G.size();
    vector<long long> dist(N, INF_ll);
    dist[s_0] = 0;
    ...
    que.push(make_pair(dist[s_0], s_0));
    que.push(make_pair(dist[s_1], s_1));
...
int main()
{
    ...
    vector<long long> dist_0 = BFS_01(G, s_0);
    vector<long long> dist_1 = BFS_01(G, s_1);

    long long ans = INF_ll;
    chmin(ans, dist_0[t_0]);
    chmin(ans, dist_0[t_1]);
    chmin(ans, dist_1[t_0]);
    chmin(ans, dist_1[t_1]);
