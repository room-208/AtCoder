...
int main()
{
    ...
    int d_R = (int)floor(log10((long double)R));
    int d_L = (int)floor(log10((long double)L));

    long long ans_R = 0;
    for (int i = 0; i <= d_R; i++)
    {
        long long ten_pow_1 = pow(10, i);
        long long ten_pow_2 = pow(10, i + 1);

        if (i == d_R)
        {
            ans_R += sum(ten_pow_1, R) * (i + 1);
            ans_R %= MOD;
        }
        else
        {
            ans_R += sum(ten_pow_1, ten_pow_2 - 1) * (i + 1);
            ans_R %= MOD;
        }
    }
    ...
    for (int i = 0; i <= d_L; i++)
    {
        ...
        if (i == d_L)
        {
            ans_L += sum(ten_pow_1, L) * (i + 1);
            ans_L %= MOD;
        }
        else
        {
            ans_L += sum(ten_pow_1, ten_pow_2 - 1) * (i + 1);
            ans_L %= MOD;
        }
    }

    long long ans = ans_R - ans_L;
