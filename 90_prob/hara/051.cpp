...
int main()
{
    ...
    int M = N / 2;
    vector<long long> A_1(M);
    vector<long long> A_2(N - M);
    vector<vector<long long>> val_1(M + 1);
    vector<vector<long long>> val_2(N - M + 1);
    ...
    for (int n = 0; n <= M; n++)
    {
        if (!val_1[n].empty())
        {
            sort(val_1[n].begin(), val_1[n].end());
        }
    }
    //val_2ソート
    for (int n = 0; n <= N - M; n++)
    {
        if (!val_2[n].empty())
        {
            sort(val_2[n].begin(), val_2[n].end());
        }
    }
    ...
    for (int i = 1; i <= M; i++)
    {
        for (int j = 0; j < (int)val_1[i].size(); j++)
        {
            cout << i << " " << val_1[i][j] << endl;
        }
    }
    cout << "val_2" << endl;
    for (int i = 1; i <= N - M; i++)
    {
        for (int j = 0; j < (int)val_2[i].size(); j++)
        {
            cout << i << " " << val_2[i][j] << endl;
        }
    }
    */
    ...
    for (int n = 0; n <= K; n++)
    {
        if (n <= M && K - n <= N - M)
        {
            for (int i = 0; i < (int)val_1[n].size(); i++)
            {
                long long key = P - val_1[n][i];

                int m = upper_bound(val_2[K - n].begin(), val_2[K - n].end(), key) - val_2[K - n].begin();
