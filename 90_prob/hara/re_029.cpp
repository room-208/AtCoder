...
int main()
{
    ...
    vector<long long> zero(W, 0LL);
    lazy_segtree<long long, seg::op, seg::e, long long, seg::mapping, seg::composition, seg::id> sgt(zero);
    for (int i = 0; i < N; i++)
    {
        long long h = sgt.prod(L[i], R[i] + 1);

        cout << h + 1 << endl;

        sgt.apply(L[i], R[i] + 1, h + 1);
    }
}