...
// 深さ優先探索
void DFS(const Graph_int &G, int v, vector<bool> &seen, vector<int> &vec)
{
    ...
    for (auto next_v : G[v])
    {
        ...
        DFS(G, next_v, seen, vec); // 再帰的に探索
    }

    vec.push_back(v);
}

void DFS_2(const Graph_int &G, int v, vector<bool> &seen, UnionFind &uf)
{
    seen[v] = true; // v を訪問済にする
