...
// 深さ優先探索
void DFS(const Graph_int &G, int v, vector<bool> &seen, vector<int> &tpl_sort)
{
    ...
    for (auto next_v : G[v])
    {
        ...
        DFS(G, next_v, seen, tpl_sort); // 再帰的に探索
    }

    tpl_sort.push_back(v);
}
...
int main()
{
    ...
    vector<vector<long long>> S(N + 1, vector<long long>(2));
    S[0][0] = 0;
    S[0][1] = 0;
    for (int i = 0; i < N; i++)
    {
        S[i + 1][C[i]] = S[i][C[i]] + P[i];
        S[i + 1][1 - C[i]] = S[i][1 - C[i]];
    }

    for (int i = 0; i < Q; i++)
    {
        cout << S[R[i] + 1][0] - S[L[i]][0] << " " << S[R[i] + 1][1] - S[L[i]][1] << endl;
    }
}