...
// 深さ優先探索
void DFS1(const Graph_int &G, int v, vector<int> &dist, int p = -1,
          int depth = 0) {
  dist[v] = depth;
  ...
  for (auto next_v : G[v]) {
    ...
    DFS1(G, next_v, dist, v, depth + 1);
  }
}

int get_s(vector<int> &dist) {
  int s;
  int depth = -1;
...
// 深さ優先探索
void DFS2(const Graph_int &G, int v, int &ans, int p = -1, int depth = 0) {
  chmax(ans, depth);
