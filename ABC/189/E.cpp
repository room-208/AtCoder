...
int main()
{
    ...
    int M;
    cin >> M;
    vector<pair<int, long long>> op(M + 1);
    for (int i = 1; i <= M; i++)
    {
        cin >> op[i].first;
    ...
    vector<long long> ans_x(Q), ans_y(Q);
    int q = 0;
    long long x = 0;
    long long y = 0;
    double theta = 0.;
    ...
    for (int m = 0; m <= M; m++)
    {
        while (s[q].a == m)
        {
            int i = s[q].querry;
            int B = s[q].b;
            int sn = (int)sin(-theta);
            int cs = (int)cos(-theta);
            long long d_x = X[B] - x;
            long long d_y = Y[B] - y;
            d_x *= mirror_x;
            d_y *= mirror_y;
            ans_x[i] = cs * d_x - sn * d_y;
            ans_y[i] = sn * d_x + cs * d_y;
            q++;
