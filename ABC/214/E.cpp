...
long long mapping(long long f, long long x) { return x + f; }
...
int main() {
  ...
  cin >> T;
  vector<int> N(T);
  vector<vector<long long>> L(T), R(T);
  for (int i = 0; i < T; i++) {
    cin >> N[i];
    L[i].resize(N[i]);
    R[i].resize(N[i]);
  ...
  for (int i = 0; i < T; i++) {
    ...
    for (int j = 0; j < N[i]; j++) {
      p.push_back(make_pair(R[i][j], L[i][j]));
    }
    sort(p.begin(), p.end());
    for (int j = 0; j < N[i]; j++) {
      R[i][j] = p[j].first;
  ...
  for (int i = 0; i < T; i++) {
    bool flag = true;
    unordered_set<long long> full_box;
    set<long long> empty_box;
    full_box.insert(0);
    empty_box.insert(1);

    for (int j = 0; j < N[i]; j++) {
      if (!full_box.count(L[i][j])) {
        full_box.insert(L[i][j]);
        empty_box.erase(L[i][j]);
        if (!full_box.count(L[i][j] + 1)) {
          empty_box.insert(L[i][j] + 1);
        }
      } else {
        auto itr = empty_box.lower_bound(L[i][j] + 1);
        if (itr == empty_box.end()) {
          flag = false;
          break;
        } else if (*itr > R[i][j]) {
          flag = false;
          ...
          empty_box.erase(now);
          full_box.insert(now);
          if (!full_box.count(now + 1)) {
            empty_box.insert(now + 1);
          }
        }
      }
    }
