...
long long f(const vector<long long> &Y, long long n)
{
    int N = (int)Y.size();
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        //オーバーフロー
        if (__builtin_mul_overflow(Y[i], n_pow, &tmp))
        {
            flag = false;
            break;
        }
        ...
        //オーバーフロー
        if (__builtin_add_overflow(f, y, &tmp))
        {
            flag = false;
            break;
        }
        ...
        {
            //オーバーフロー
            if (__builtin_mul_overflow(n_pow, n, &tmp))
            {
                flag = false;
                break;
            }
