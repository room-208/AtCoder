...
int main() {
  ...
  vector<pair<int, int>> v1, vN;
  for (int i = 0; i < M; i++) {
    if (U[i] == -1) {
      if (d1[V[i]] != -1) {
        v1.push_back({d1[V[i]], V[i]});
      }
      if (dN[V[i]] != -1) {
        vN.push_back({dN[V[i]], V[i]});
      }
    }
  }
  ...
  for (int k = 0; k < N; k++) {
    ...
    if (dN[k] != -1) {
      if (!v1.empty()) {
        if (v1[0].second != k) {
          chmin(ans, dN[k] + 1 + v1[0].first);
        }
      }
    }
    if (!v1.empty() && !vN.empty()) {
      chmin(ans, 2 + v1[0].first + vN[0].first);
    }
