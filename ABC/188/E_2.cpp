...
// 深さ優先探索
void DFS(const Graph &G, int v, vector<bool> &seen, vector<long long> &A, long long buy, long long &score, int s)
{
    //cout << v << " " << s << endl;
    if (v != s)
    {
    ...
    for (auto next_v : G[v])
    {
        ...
        DFS(G, next_v, seen, A, buy, score, s); // 再帰的に探索
    }
}
