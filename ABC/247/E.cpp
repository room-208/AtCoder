...
int main() {
  ...
  vector<int> x_ids, y_ids, not_ids;
  for (int i = 0; i < N; i++) {
    if (X == A[i]) {
      x_ids.push_back(i);
    }
  ...
  for (int i = 0; i < N; i++) {
    if (Y <= A[i] && A[i] <= X) {
      auto x_itr = lower_bound(x_ids.begin(), x_ids.end(), i);
      auto y_itr = lower_bound(y_ids.begin(), y_ids.end(), i);
      ...
      int index = max(*x_itr, *y_itr);
      auto not_itr = lower_bound(not_ids.begin(), not_ids.end(), i);
      if (not_itr == not_ids.end()) {
        ans += N - 1 - index + 1;
      } else if (*not_itr <= index) {
        continue;
      } else {
        ans += *not_itr - 1 - index + 1;
      }
    }
  }
