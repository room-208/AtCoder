...
long long COMV(int i, int N, long long V) {
  if (i == 0) {
    long long Q = V - 1;
    if (Q <= 1) {
      return 0;
...
long long COMH(int i, int N, long long H) {
  if (i == 0) {
    long long Q = H - 1;
    if (Q <= 0) {
      return 0;
...
int main() {
  ...
  long long cnt = 0, cnt_new = 0;
  long long Value = -1;
  for (long long V = 3; V <= 3LL * N; V++) {
    cnt_new = cnt + COMV(0, N, V) - 3LL * COMV(1, N, V) + 3LL * COMV(2, N, V) -
              COMV(3, N, V);
    if (cnt_new >= K) {
      Value = V;
      break;
  ...
  for (long long i = 1; i <= N; i++) {
    long long H = Value - i;
    cnt_new = cnt + COMH(0, N, H) - 2LL * COMH(1, N, H) + COMH(2, N, H);
    if (cnt_new >= K) {
      a = i;
      break;
    } else {
