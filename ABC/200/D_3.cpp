...
void ans_out(int i, int n, vector<int> &ans, vector<int> &A, vector<vector<bool>> &dp)
{
    while (1)
    {
        if (i == 0 || n == 0)
...
int main()
{
    ...
    //実現可能な場合の数
    vector<vector<bool>> dp(201, vector<bool>(N + 1, false));
    dp[0][0] = true;
    int tmp_i;
    int tmp_n;
    bool flag = false;
    ...
    else
    {
        ...
        {
            ans.clear();
            ans_out(i - 1, n, ans, A, dp);
            cnt++;
        }
        //0から実現される場合
        if (back == 200 && dp[0][i - 1] && cnt != 2)
        {
            ans.clear();
            ans.push_back(i);
            ans_out(i - 1, 0, ans, A, dp);
        }
    }
}