...
int main() {
  ...
  long long left = -2LL * INF_ll;
  long long right = 2LL * INF_ll;
  while (right - left > 1) {
    long long x = (right + left) / 2;
    long long cnt = 0;
    for (int i = 0; i < N; i++) {
      if (A[i] > 0) {
        long long key = my_floor(x, A[i]);
        int n = upper_bound(A.begin(), A.end(), key) - A.begin();
        n--;
        if (A[n] <= key) {
          if (i < n) {
            cnt += n - i;
    ...
    int m = upper_bound(B.begin(), B.end(), x) - B.begin();
    cout << m << " ";
    cout << cnt << " ";
    if (m == cnt) {
      cout << "yes" << endl;
