...
vector<vector<int>> Getvec(vector<int> t, vector<int> i, vector<int> j) {
  int Q = t.size();
...
int main() {
  int N, M, Q;
  cin >> N >> M >> Q;
  vector<int> t(Q), l(Q), r(Q), i(Q), j(Q);
  vector<ll> x(Q);
  for (int q = 0; q < Q; q++) {
    cin >> t[q];
    if (t[q] == 1) {
  ...
  vector<vector<int>> vec = Getvec(t, i, j);
  ...
  for (int q = 0; q < Q; q++) {
    if (t[q] == 1) {
      ...
      cout << mp[i[q]][j[q]] + sgt.get(j[q]) << endl;
    }
  }
}