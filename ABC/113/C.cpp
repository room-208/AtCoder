...
int main()
{
    ...
    vector<vector<pair<int, int>>> ken(N);
    for (int i = 0; i < M; i++)
    {
        ken[P[i]].push_back(make_pair(Y[i], i));
    }
    for (int i = 0; i < N; i++)
    {
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        for (int j = 0; j < (int)ken[i].size(); j++)
        {
            string f = to_string(i + 1);
            string s = to_string(j + 1);
            ans[ken[i][j].second] = make_pair(f, s);
        }
    }

    vector<string> zero(6);
    zero[1] = "0";
    zero[2] = "00";
    zero[3] = "000";
    zero[4] = "0000";
    ...
    for (int i = 0; i < M; i++)
    {
        int n1 = (int)ans[i].first.size();
        int n2 = (int)ans[i].second.size();
        cout << zero[6 - n1] << ans[i].first << zero[6 - n2] << ans[i].second << endl;
    }
}