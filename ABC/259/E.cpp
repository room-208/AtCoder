...
int main() {
  ...
  unordered_map<ll, multiset<ll>> mp;
  for (int i = 0; i < N; i++) {
    for (int j = 0; j < m[i]; j++) {
      mp[p[i][j]].insert(e[i][j]);
    }
  }

  unordered_map<ll, ll> mp_origin;
  for (auto &&p : mp) {
    ll val = *prev(p.second.end());
    mp_origin[p.first] = val;
  }
  ...
  for (int i = 0; i < N; i++) {
    ...
    for (int j = 0; j < m[i]; j++) {
      mp[p[i][j]].erase(mp[p[i][j]].find(e[i][j]));
      if (mp[p[i][j]].empty()) {
        vec.push_back({p[i][j], 0});
      } else {
        ll val = *prev(mp[p[i][j]].end());
        if (val != mp_origin[p[i][j]]) {
          vec.push_back({p[i][j], val});
        }
      }
      mp[p[i][j]].insert(e[i][j]);
    }
