...
int main()
{
    ...
    else
    {
        ...
        vector<vector<long long>> tate(W), yoko(H);
        for (int i = 0; i < M; i++)
        {
        ...
        {
            yoko[x].push_back(W);
            sort(yoko[x].begin(), yoko[x].end());
        }
        for (int y = 0; y < W; y++)
        {
            tate[y].push_back(H);
            sort(tate[y].begin(), tate[y].end());
        }
        ...
        fenwick_tree<long long> fw(W + 1);
        for (int x = 1; x < MAX_X; x++)
        {
            if (x == 1)
            {
                for (int y = yoko[0][0]; y < W; y++)
                {
                    fw.add(y, 1);
                }
            }
