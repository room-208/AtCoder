...
// 深さ優先探索
void DFS(const Graph_Edge &G, int v, vector<long long> &dist, vector<bool> &seen, bool &flag)
{
    seen[v] = true;
    ...
    for (auto next_v : G[v])
    {
        if (seen[next_v.to])
        {
            if (dist[v] + next_v.w != dist[next_v.to])
            {
                flag = false;
            }
        ...
        dist[next_v.to] = dist[v] + next_v.w;
        DFS(G, next_v.to, dist, seen, flag);
    }
}
