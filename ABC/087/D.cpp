...
// 深さ優先探索
void DFS(const Graph_Edge &G, int v, vector<bool> &seen, vector<long long> &dist, bool &flag)
{
    seen[v] = true;
    ...
    for (auto next_v : G[v])
    {
        if (seen[next_v.to])
        {
            if (dist[next_v.to] != dist[v] + next_v.w)
            {
                flag = false;
            }
        ...
        DFS(G, next_v.to, seen, dist, flag);
    }
}
