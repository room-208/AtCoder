...
ll func(ll a, ll b) {
  ll z;
  ll a2, a3, b2, b3, a2b, b2a;

  if (__builtin_mul_overflow(a, a, &z)) {
    return -1;
  } else {
    a2 = a * a;
  }
  if (__builtin_mul_overflow(a, a2, &z)) {
    return -1;
  } else {
    a3 = a * a2;
  }
  if (__builtin_mul_overflow(b, b, &z)) {
    return -1;
  } else {
    b2 = b * b;
  }
  if (__builtin_mul_overflow(b, b2, &z)) {
    return -1;
  } else {
    b3 = b * b2;
  }
  if (__builtin_mul_overflow(a2, b, &z)) {
    return -1;
  } else {
    a2b = a2 * b;
  }
  if (__builtin_mul_overflow(b2, a, &z)) {
    return -1;
  } else {
    b2a = b2 * a;
  }
  ...
  ll x = 0;
  if (__builtin_add_overflow(x, a3, &z)) {
    return -1;
  } else {
    x += a3;
  }
  if (__builtin_add_overflow(x, b3, &z)) {
    return -1;
  } else {
    x += b3;
  }
  if (__builtin_add_overflow(x, a2b, &z)) {
    return -1;
  } else {
    x += a2b;
  }
  if (__builtin_add_overflow(x, b2a, &z)) {
    return -1;
  } else {
    x += b2a;
  }
...
ll f(ll a, ll b) { return a * a * a + b * a * a + b * b * a + b * b * b; }
