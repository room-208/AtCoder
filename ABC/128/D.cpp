...
//セグ木・遅延セグ木
const int sgt_ID = -1;
int sgt_op(int a, int b) { return max(a, b); }
int sgt_e() { return -INF_int; }
int sgt_mapping(int f, int x)
{
    if (f == sgt_ID)
    {
        return x;
...
int sgt_composition(int f, int g)
{
    if (f == sgt_ID)
    {
        return g;
...
int main()
{
    ...
    vector<vector<vector<long long>>> dp(K + 1, vector<vector<long long>>(N + 1, vector<long long>(2, -INF_ll)));
    ...
    for (int j = 1; j <= N; j++)
    {
        for (int i = 0; i <= K; i++)
        {
            if (i - 1 >= 0)
            {
                if (dp[i - 1][j - 1][0] != -INF_ll)
                {
                    chmax(dp[i][j][0], dp[i - 1][j - 1][0] + V[j - 1]);
                }
            }

            if (i - 2 >= 0)
            {
                if (dp[i - 2][j - 1][0] != -INF_ll)
                {
                    chmax(dp[i][j][0], dp[i - 2][j - 1][0]);
                }
            }
        }
    }
    ...
    for (int j = 1; j <= N; j++)
    {
        for (int i = 0; i <= K; i++)
        {
            if (i - 1 >= 0)
            {
                if (dp[i - 1][j - 1][1] != -INF_ll)
                {
                    chmax(dp[i][j][1], dp[i - 1][j - 1][1] + V[j - 1]);
                }
            }
