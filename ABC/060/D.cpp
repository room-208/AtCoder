...
int main()
{
    ...
    vector<vector<vector<long long>>> dp(N + 1, vector<vector<long long>>(N + 1, vector<long long>(4, -1)));
    for (int k = 0; k < 4; k++)
    {
        dp[0][0][k] = 0;
    }
    for (int k = 0; k < 4; k++)
    {
        for (int j = 1; j <= N; j++)
        {
            ...
            for (int i = 1; i <= N; i++)
            {
                if (dp[i - 1][j - 1][k] != -1)
                {
                    chmax(dp[i][j][k], dp[i - 1][j - 1][k] + A[k][j]);
                }
            }
        }
    }
    ...
    for (int i = 0; i < (int)A[0].size(); i++)
    {
        for (int j = 0; j < (int)A[1].size(); j++)
        {
            for (int k = 0; k < (int)A[2].size(); k++)
            {
                ...
                {
                    long long S = w[0] * i + (w[0] + 1) * j + (w[0] + 2) * k + (w[0] + 3) * l;

                    if (S <= W)
                    {
                        chmax(ans, dp[i][N][0] + dp[j][N][1] + dp[k][N][2] + dp[l][N][3]);
                    }
                }
            }
        }
