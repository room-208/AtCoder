...
int main()
{
    ...
    for (int i = 1; i <= H; i++)
    {
        ...
        {
            int s = S[i - 1][j - 1] - '0';
            A[i][j] = A[i][j - 1] + A[i - 1][j] - A[i - 1][j - 1] + s;
        }
    }
    /*
    for (int i = 0; i <= H; i++)
    ...
    */
    ...
    for (int bit = 0; bit < BIT; bit++)
    {
        ...
        for (int j = 1; j <= W; j++)
        {
            int j_1 = *prev(tate.end());
            int j_2 = j;

            for (int i = 0; i < (int)yoko.size() - 1; i++)
            {
                int i_1 = yoko[i];
                int i_2 = yoko[i + 1];

                int C = A[i_2][j_2] - A[i_1][j_2] - A[i_2][j_1] + A[i_1][j_1];
        ...
        if (flag)
        {
            chmin(ans, (int)yoko.size() + (int)tate.size() - 3);
        }
    }
