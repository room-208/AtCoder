...
int main() {
  ...
  tie(A, index) = GetSortedPair(A, index, true);
  ...
  for (int i = X; i < N; i++) {
    ...
    b.push_back(B[-index[i]]);
  }

  tie(b, index2) = GetSortedPair(b, index2, true);
  ...
  vector<int> index3, c;
  for (int i = Y; i < index2.size(); i++) {
    index3.push_back(index2[i]);
    c.push_back(C[-index2[i]]);
  }

  tie(c, index3) = GetSortedPair(c, index3, true);
