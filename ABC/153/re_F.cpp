...
int main()
{
    ...
    lazy_segtree<long long, seg::op, seg::e, long long, seg::mapping, seg::composition, seg::id> sgt(H);
    long long cnt = 0;
    for (int i = 0; i < N; i++)
    {
        long long v = sgt.get(i);
        ...
        long long key = X[i] + 2LL * D;
        auto r = upper_bound(X.begin(), X.end(), key) - X.begin();
        sgt.apply(i, r, -A * tmp);
    }
