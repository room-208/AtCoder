...
int main() {
  ...
  vector<int> h1(Q), h2(Q), w1(Q), w2(Q);
  for (int i = 0; i < Q; i++) {
  ...
  Segtree sgt_C(C), sgt_D(D);
  for (int i = 0; i < Q; i++) {
    ll val_A, val_B;
    if (h1[i] == h2[i] && w1[i] == w2[i]) {
      cout << A[h1[i]] + B[w1[i]] << endl;
    } else if (h1[i] == h2[i]) {
      val_B = sgt_D.prod(w1[i], w2[i]);
      val_B = GCD(val_B, A[h1[i]] + B[w1[i]]);
      cout << val_B << endl;
    } else if (w1[i] == w2[i]) {
      val_A = sgt_C.prod(h1[i], h2[i]);
      val_A = GCD(val_A, A[h1[i]] + B[w1[i]]);
      cout << val_A << endl;
    } else {
      val_A = sgt_C.prod(h1[i], h2[i]);
      val_A = GCD(val_A, A[h1[i]] + B[w1[i]]);
