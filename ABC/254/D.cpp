...
void dfs(vector<ll> &key, unordered_map<ll, int> &mp, ll N, ll &ans, ll a = 1,
         ll b = 1, int depth = 0) {
  if (depth == mp.size()) {
    if (a <= N && b <= N) {
      ans++;
  ...
  for (int i = 0; i <= mp[key[depth]]; i++) {
    dfs(key, mp, N, ans, a * pow(key[depth], i),
        b * pow(key[depth], mp[key[depth]] - i), depth + 1);
  }
}
