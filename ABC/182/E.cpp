...
int main()
{
    ...
    set<pair<int, int>> st_l, st_b;
    for (int i = 0; i < N; i++)
    {
        st_l.insert(make_pair(A[i], B[i]));
    }
    for (int i = 0; i < M; i++)
    {
        st_b.insert(make_pair(C[i], D[i]));
    }
    ...
    for (int i = 0; i < H; i++)
    {
        for (int j = 0; j < W; j++)
        {
            ...
            if (!row[i].empty())
            {
                int n_row = lower_bound(row[i].begin(), row[i].end(), j) - row[i].begin();

                if (n_row == 0)
                {
                    if (st_l.count(make_pair(i, row[i][n_row])))
                    {
                        cnt++;
                        flag = true;
                    }
                }
                else if (n_row == (int)row[i].size())
                {
                    if (st_l.count(make_pair(i, row[i][n_row - 1])))
                    {
                        cnt++;
                        flag = true;
                    }
                ...
                else
                {
                    ...
                    else if (st_l.count(make_pair(i, row[i][n_row])))
                    {
                        cnt++;
                        flag = true;
                    }
            ...
            if (!flag)
            {
                if (!col[j].empty())
                {
                    int n_col = lower_bound(col[j].begin(), col[j].end(), i) - col[j].begin();

                    if (n_col == 0)
                    {
                        if (st_l.count(make_pair(col[j][n_col], j)))
                        {
                            cnt++;
                        }
                    }
                    else if (n_col == (int)col[j].size())
                    {
                        if (st_l.count(make_pair(col[j][n_col - 1], j)))
                        {
                            cnt++;
                        }
                    }
                    else
                    {
                        ...
                        else if (st_l.count(make_pair(col[j][n_col], j)))
                        {
                            cnt++;
                        }
                    }
