...
vector<vector<ll>> warshall_floyd(int N, int M, vector<int> &a, vector<int> &b,
                                  vector<ll> c) {
  vector<vector<ll>> d(N, vector<ll>(N, INF_ll));
  for (int i = 0; i < M; i++) {
    d[a[i]][b[i]] = c[i];
    d[b[i]][a[i]] = c[i];
  }
...
int main() {
  ...
  vector<vector<ll>> d = warshall_floyd(N, M, A, B, C);
