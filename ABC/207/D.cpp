...
int main()
{
    ...
    vector<double> dist_ab, dist_cd;
    for (int i = 0; i < N - 1; i++)
    {
        for (int j = i + 1; j < N; j++)
        {
            double tmp = dist(a[i], b[i], a[j], b[j]);
            dist_ab.push_back(tmp);
        }
    }
    for (int i = 0; i < N - 1; i++)
    {
        for (int j = i + 1; j < N; j++)
        {
            double tmp = dist(c[i], d[i], c[j], d[j]);
            dist_cd.push_back(tmp);
        }
    }

    sort(dist_ab.begin(), dist_ab.end());
    sort(dist_cd.begin(), dist_cd.end());

    bool flag = true;
    for (int i = 0; i < dist_cd.size(); i++)
    {
        if (dist_ab[i] != dist_cd[i])
        {
            flag = false;
            break;
        }
