...
void Ans(int K, vector<pair<long long, int>> &p, mint ans = 1) {
  for (int i = 0; i < K; i++) {
    ans *= p[i].first;
  }
  cout << ans.val() << endl;
...
int main() {
  ...
  if (cnt % 2 == 0) {
    ...
    long long min_A_p = INF_ll;
    int index_min_p;
    long long min_A_m = INF_ll;
    int index_min_m;
    for (int i = 0; i < K; i++) {
      if (p[i].second == 1) {
        if (chmin(min_A_p, p[i].first)) {
          index_min_p = i;
        }
      } else {
        if (chmin(min_A_m, p[i].first)) {
          index_min_m = i;
        }
      }
    }
    ...
    for (int i = K; i < (int)p.size(); i++) {
      if (p[i].second == 1) {
        if (chmax(max_A_p, p[i].first)) {
          index_max_p = i;
        }
      } else {
        if (chmax(max_A_m, p[i].first)) {
          index_max_m = i;
        }
      }
    }

    if (min_A_p == INF_ll || max_A_m == -1) {
      if (min_A_m == INF_ll || max_A_p == -1) {
        if ((int)p.size() < N) {
          cout << 0 << endl;
          return 0;
        } else {
        ...
        swap(p[index_min_m], p[index_max_p]);
        Ans(K, p);
      }
    } else {
      if (min_A_m == INF_ll || max_A_p == -1) {
        swap(p[index_min_p], p[index_max_m]);
        Ans(K, p);
      } else {
        if (max_A_p * min_A_p < max_A_m * min_A_m) {
          swap(p[index_min_p], p[index_max_m]);
          Ans(K, p);
        } else {
          swap(p[index_min_m], p[index_max_p]);
