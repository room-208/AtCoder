...
long long get_median(vector<long long> &A, int N) {
  ...
  vector<long long> vec_median;
  int index_median = 0;
  while (!st.empty()) {
    vec_median.push_back(p[index_median].first);
    int i = p[index_median].second;
    st.erase(i - 1);
    st.erase(i);
    st.erase(i + 1);
    index_median++;
  ...
  long long median = vec_median[ceil((double)vec_median.size() / 2.) - 1];
  return median;
}
