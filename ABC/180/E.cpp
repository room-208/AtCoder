...
long long Cost_i_to_j(int i, int j, vector<long long> &X, vector<long long> &Y, vector<long long> &Z)
{
    return llabs(X[j] - X[i]) + llabs(Y[j] - Y[i]) + max(0LL, Z[j] - Z[i]);
}
...
int main()
{
    ...
    vector<vector<long long>> dp(S, vector<long long>(N, INF_ll));
    dp[0][0] = 0LL;
    ...
    for (int bit = 0; bit < S; bit++)
    {
        for (int i = 0; i < N; i++)
        {
            for (int j = 0; j < N; j++)
            {
                ...
                long long c_ij = Cost_i_to_j(i, j, X, Y, Z);
                int next_bit = bit | (1 << j);
                chmin(dp[next_bit][j], dp[bit][i] + c_ij);
            }
        }
    }
