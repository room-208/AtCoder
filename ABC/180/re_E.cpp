...
long long cost(int i, int j, vector<long long> &X, vector<long long> &Y, vector<long long> &Z)
{
    ...
    for (int bit = 0; bit < (1 << N); bit++)
    {
        for (int i = 0; i < N; i++)
        {
            for (int j = 0; j < N; j++)
            {
                ...
                cout << bitset<10>(bit) << " " << bitset<10>(bit & (1 << j)) << endl;

                chmin(dp[bit | (1 << i)][i], dp[bit][j] + cost(j, i, X, Y, Z));
            }
        }
    }
