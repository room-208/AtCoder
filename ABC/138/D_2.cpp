...
// 深さ優先探索
void DFS(const Graph_int &G, int v, const vector<int> &dist, const vector<long long> &y, vector<long long> &sum, vector<long long> &counter)
{
    if (v == 0)
    {
        sum[v] += y[v];
    ...
    {
        //親側
        if (dist[next_v] <= dist[v])
        {
            continue;
        }

        sum[next_v] = sum[v] + y[next_v];
        counter[next_v] += sum[next_v];

        DFS(G, next_v, dist, y, sum, counter); // 再帰的に探索
    }
}
