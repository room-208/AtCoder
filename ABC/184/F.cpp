...
int main()
{
    ...
    vector<long long> A_1(N_1);
    vector<long long> A_2(N_2);
    for (int i = 0; i < N_1; i++)
    {
        A_1[i] = A[i];
    }
    ...
    vector<long long> p_1;
    for (int bit = 0; bit < (1 << N_1); bit++)
    {
        long long tmp = 0;
        for (int i = 0; i < N_1; i++)
        {
    ...
    vector<long long> p_2;
    for (int bit = 0; bit < (1 << N_2); bit++)
    {
        long long tmp = 0;
        for (int i = 0; i < N_2; i++)
        {
    ...
    sort(p_1.begin(), p_1.end());
    sort(p_2.begin(), p_2.end());
    int size_1 = (int)p_1.size();
    int size_2 = (int)p_2.size();

    long long ans = 0;
    for (int i_1 = 0; i_1 < size_1; i_1++)
    {
        long long key = T - p_1[i_1];
        int tmp = upper_bound(p_2.begin(), p_2.end(), key) - p_2.begin();
        int i_2 = tmp - 1;

        if (0 <= i_2 && i_2 < size_2)
        {
            long long ans_tmp = p_1[i_1] + p_2[i_2];
            chmax(ans, ans_tmp);
        }
    }
