...
int main()
{
    ...
    for (int i = 0; i < N; i++)
    {
        if (x[i] == 1.)
        {
            p.push_back(make_pair((long double)(2 * INF_int), (y[i] - 1.) / x[i]));
        }
        else
        {
            p.push_back(make_pair(y[i] / (x[i] - 1.), (y[i] - 1.) / x[i]));
        }
    }
    sort(p.begin(), p.end());
    ...
    {
        long double next_left = p[i].second;
        long double next_right = p[i].first;
