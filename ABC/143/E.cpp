...
//ダイクストラ法
vector<pair<long long, long long>> Dijkstra(const Graph_Edge &G, long long L,
                                            int s) {
  int N = (int)G.size();
  vector<pair<long long, long long>> dist(N, make_pair(INF_ll, 0));
  ...
  while (!que.empty()) {
    ...
    auto dist_top = que.top().first;
    long long d = dist_top.first;
    long long l = -dist_top.second;
    que.pop();

    // d > dist[v] は，(d, v) がゴミであることを意味する
    if (dist_top > dist[v]) continue;

    // 頂点 v を始点とした各辺を緩和
    for (auto e : G[v]) {
      if (l - e.w >= 0) {
        auto dist_new = make_pair(d, -(l - e.w));
        if (chmin(dist[e.to], dist_new)) {
          // 更新があるならヒープに新たに挿入
          que.push(make_pair(dist_new, e.to));
        }
      } else {
        if (L >= e.w) {
          auto dist_new = make_pair(d + 1, -(L - e.w));
          if (chmin(dist[e.to], dist_new)) {
            // 更新があるならヒープに新たに挿入
            que.push(make_pair(dist_new, e.to));
          }
...
int main() {
  ...
  for (int v = 0; v < N; v++) {
    vec.push_back(Dijkstra(G, L, v));
  }

  for (int i = 0; i < Q; i++) {
    long long ans = vec[s[i]][t[i]].first;
    if (ans == INF_ll) {
      cout << -1 << endl;
    } else {
      cout << ans << endl;
