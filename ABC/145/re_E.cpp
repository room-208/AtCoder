...
int main()
{
    ...
    vector<vector<pair<long long, long long>>> dp(T, vector<pair<long long, long long>>(N + 1, make_pair(-1LL, -1LL)));
    ...
    for (int j = 1; j <= N; j++)
    {
        ...
        for (int i = 0; i < T; i++)
        {
            if (dp[i][j - 1] != make_pair(-1LL, -1LL))
            {
                //採用する場合
                if (i + A[j] < T)
                {
                    if (dp[i + A[j]][j].first + dp[i + A[j]][j].second < dp[i][j - 1].first + dp[i][j - 1].second + B[j])
                    {
                        dp[i + A[j]][j] = make_pair(dp[i][j - 1].first + B[j], dp[i][j - 1].second);
                    }
                }

                //採用しない場合
                if (dp[i][j].first + dp[i][j].second < dp[i][j - 1].first + B[j])
                {
                    if (dp[i][j].second < B[j])
                    {
                        dp[i][j] = make_pair(dp[i][j - 1].first, B[j]);
                    }
                }
            }
        }
