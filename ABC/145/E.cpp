...
int main()
{
    ...
    vector<vector<long long>> dp(T + 1, vector<long long>(N + 1, -INF_int));
    dp[0][0] = 0;
    ...
    for (int j = 1; j <= N; j++)
    {
        for (int i = 0; i <= T; i++)
        {
            ...
            if (i - A[j] >= 0)
            {
                if (dp[i - A[j]][j - 1] != -INF_int)
                {
                    chmax(dp[i][j], dp[i - A[j]][j - 1] + B[j]);
                }
            }
        }
    }
    ...
    */
    ...
    vector<vector<long long>> his(T + 1);
    ...
    for (int i = 0; i <= T; i++)
    {
        ...
        while (j_tmp != 0)
        {
            if (i_tmp - A[j_tmp] >= 0)
            {
                if (dp[i_tmp][j_tmp - 1] < dp[i_tmp - A[j_tmp]][j_tmp - 1] + B[j_tmp])
                {
                    his[i].push_back(j_tmp);
                    i_tmp = i_tmp - A[j_tmp];
                }
            }
    ...
    for (int i = 0; i <= T; i++)
    {
        ...
        {
            cout << i << " ";
            for (int k = 0; k < (int)his[i].size(); k++)
            {
                cout << his[i][k] << " ";
            }
            cout << endl;
    ...
    */
    ...
    long long ans = dp[T][N];
    unordered_set<long long> set;
    for (int i = 0; i < T; i++)
    {
        if (dp[i][N] != -INF_int)
        {
            long long ans_tmp = dp[i][N];
