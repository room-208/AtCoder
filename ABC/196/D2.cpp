...
void dfs(int H, int W, int i, int j, int A, int B, vector<vector<bool>> seen,
         int &ans) {
  if (i == H - 1 && j == W - 1) {
    ans++;
    return;
  ...
  if (seen[i][j]) {
    if (i + 1 < H) {
      dfs(H, W, i + 1, j, A, B, seen, ans);
    } else {
      dfs(H, W, 0, j + 1, A, B, seen, ans);
    }
    return;
  }
  ...
  if (B > 0) {
    seen[i][j] = true;
    if (i + 1 < H) {
      dfs(H, W, i + 1, j, A, B - 1, seen, ans);
    } else {
      dfs(H, W, 0, j + 1, A, B - 1, seen, ans);
    }
    seen[i][j] = false;
  }
  ...
  if (A > 0) {
    if (i + 1 < H) {
      if (!seen[i + 1][j]) {
        ...
        seen[i + 1][j] = true;
        if (i + 1 < H) {
          dfs(H, W, i + 1, j, A - 1, B, seen, ans);
        } else {
          dfs(H, W, 0, j + 1, A - 1, B, seen, ans);
        }
        seen[i][j] = false;
        seen[i + 1][j] = false;
      }
