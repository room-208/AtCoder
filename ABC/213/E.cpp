...
void Cal_c(vector<vector<char>> &S, int i, int j, long long &c)
{
    if (S[i][j] == '#')
    {
        c = 1;
...
bool in_out(int i, int j, const int H, const int W)
{
    if (0 <= i && i < H && 0 <= j && j < W)
    {
        return true;
...
int main()
{
    ...
    for (int i = 0; i < H; i++)
    {
        for (int j = 0; j < W; j++)
        {
            ...
            if (in_out(i - 1, j, H, W))
            {
                if (S[i - 1][j] == '#')
                {
                    for (int k_i = -2; k_i <= -1; k_i++)
                    {
                        for (int k_j = -1; k_j <= 1; k_j++)
                        {
                            if (in_out(i + k_i, j + k_j, H, W))
                            {
                                v = a[i + k_i][j + k_j];
    ...
    vector<long long> dist = BFS_01(G, 0);
