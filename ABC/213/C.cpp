...
int main()
{
    ...
    sort(h_a.begin(), h_a.end());
    sort(h_b.begin(), h_b.end());

    vector<Hara> h_a_tmp = h_a, h_b_tmp = h_b;

    h_a[0].n = 1;
    h_b[0].n = 1;
    for (int i = 1; i < N; i++)
    {
        if (h_a[i].n != h_a_tmp[i - 1].n)
        {
            h_a[i].n = h_a[i - 1].n + 1;
        }
        else
    ...
    for (int i = 0; i < N; i++)
    {
        p[h_a[i].i].first = h_a[i].n;
        p[h_b[i].i].second = h_b[i].n;
    }
