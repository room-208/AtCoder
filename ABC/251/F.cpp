...
// 深さ優先探索
void DFS1(const Graph_int &G, vector<bool> &seen, vector<pair<int, int>> &T1,
          int v = 0) {
  seen[v] = true;
  ...
  for (auto next_v : G[v]) {
    ...
    T1.push_back({v, next_v});
    DFS1(G, seen, T1, next_v);
  }
}

//幅優先探索
vector<int> BFS(const Graph_int &G, vector<pair<int, int>> &T2, int s = 0) {
  int N = (int)G.size();    // 頂点数
