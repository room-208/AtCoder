...
int main() {
  ...
  vector<vector<ll>> dp0(N, vector<ll>(2, INF_ll));
  dp0[1][1] = A[0];
  for (int i = 2; i < N; i++) {
    chmin(dp0[i][1], dp0[i - 1][1] + A[i - 1]);
    chmin(dp0[i][1], dp0[i - 1][0] + A[i - 1]);
    chmin(dp0[i][0], dp0[i - 1][1]);
  }
  chmin(ans, dp0[N - 1][1]);

  vector<vector<ll>> dp1(N, vector<ll>(2, INF_ll));
  dp1[0][1] = A[N - 1];
  for (int i = 1; i < N; i++) {
    chmin(dp1[i][1], dp1[i - 1][1] + A[i - 1]);
    chmin(dp1[i][1], dp1[i - 1][0] + A[i - 1]);
    chmin(dp1[i][0], dp1[i - 1][1]);
  }
  chmin(ans, dp1[N - 1][1]);
  chmin(ans, dp1[N - 1][0]);
