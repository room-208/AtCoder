...
int main()
{
    ...
    {
        S[1] = B[1] * i1;
        m[7] = min(((N - S[1]) / B[7]), n[7]);

        for (int i7 = m[7]; i7 >= 0; i7--)
        {
            S[7] = S[1] + B[7] * i7;
            m[4] = min(((N - S[7]) / B[4]), n[4]);

            for (int i4 = m[4]; i4 >= 0; i4--)
            {
                S[4] = S[7] + B[4] * i4;
                m[5] = min(((N - S[4]) / B[5]), n[5]);

                for (int i5 = m[5]; i5 >= 0; i5--)
                {
                    S[5] = S[4] + B[5] * i5;
                    m[3] = min(((N - S[5]) / B[3]), n[3]);

                    for (int i3 = m[3]; i3 >= 0; i3--)
                    {
                        S[3] = S[5] + B[3] * i3;
                        m[2] = min(((N - S[3]) / B[2]), n[2]);

                        for (int i2 = m[2]; i2 >= 0; i2--)
                        {
                            S[2] = S[3] + B[2] * i2;
                            m[9] = min(((N - S[2]) / B[9]), n[9]);

                            for (int i9 = m[9]; i9 >= 0; i9--)
                            {
                                S[9] = S[2] + B[9] * i9;
                                m[6] = min(((N - S[9]) / B[6]), n[6]);

                                for (int i6 = m[6]; i6 >= 0; i6--)
                                {
                                    S[6] = S[9] + B[6] * i6;
                                    m[8] = min(((N - S[6]) / B[8]), n[8]);

                                    for (int i8 = m[8]; i8 >= 0; i8--)
                                    {
                                        S[8] = S[6] + B[8] * i8;
