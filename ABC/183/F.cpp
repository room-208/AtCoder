    ...
    for (int q = 0; q < Q; q++)
    {
        if (T[q] == 1)
        {
            ...
            if (r == r_a)
            {
                for (auto u : v[r_b])
                {
                    auto p_a = make_pair(r, u);
                    auto p_b = make_pair(r_b, u);
            ...
            else if (r == r_b)
            {
                for (auto u : v[r_a])
                {
                    auto p_a = make_pair(r_a, u);
                    auto p_b = make_pair(r, u);
        ...
        {
            int r = uf.root(x[q]);
            if (mp.count(make_pair(r, y[q])))
            {
                cout << mp[make_pair(r, y[q])] << endl;
            }
            else
            {
                cout << 0 << endl;
