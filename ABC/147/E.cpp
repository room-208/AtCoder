...
int main() {
  ...
  // dp
  vector<vector<vector<int>>> dp(H, vector<vector<int>>(W));
  // (0,0)での初期値
  dp[0][0].emplace_back(C[0][0]);
  for (int i = 0; i < H; i++) {
    for (int j = 0; j < W; j++) {
      // しきい値
      int P = 80 * (H - i) + 80 * (W - j);
      ...
      if (j - 1 >= 0) {
        for (auto &&v : dp[i][j - 1]) {
          int value_p = abs(v + C[i][j]);
          int value_m = abs(v - C[i][j]);
          if (cnt[value_p] == 0) {
            if (value_p <= P) {
              dp[i][j].emplace_back(value_p);
              cnt[value_p]++;
            }
          }
          if (cnt[value_m] == 0) {
            if (value_m <= P) {
              dp[i][j].emplace_back(value_m);
              cnt[value_m]++;
            }
          }
        }
