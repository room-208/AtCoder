...
//連長圧縮
deque<pair<char, int>> runLengthEncoding(string s)
{
    int n = s.length();
...
int main()
{
    ...
    deque<pair<char, int>> p = runLengthEncoding(s);
    unordered_set<int> st_x, st_y;

    st_y.insert(0);
    if (p[0].first == 'F')
    {
        st_x.insert(p[0].second);
        p.pop_front();
    }
    else
    {
    ...
    for (int i = 0; i < (int)p.size(); i++)
    {
        if (p[i].first == 'F')
        {
            if (direction == 0)
            {
                ...
                for (auto v : vec)
                {
                    if (abs(v + p[i].second) <= 8000)
                    {
                        st_y.insert(v + p[i].second);
                    }
                    if (abs(v - p[i].second) <= 8000)
                    {
                        st_y.insert(v - p[i].second);
