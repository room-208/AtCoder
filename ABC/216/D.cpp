...
int main()
{
    ...
    set<pair<int, int>> st;
    vector<vector<int>> c_to_t(N);
    for (int i = 0; i < N; i++)
    {
        auto p_0 = make_pair(0, i);
        st.insert(p_0);
    }
    for (int i = 0; i < M; i++)
    {
        int c = a[i][0];
        auto p_0 = make_pair(0, c);
        auto p_1 = make_pair(1, c);
        auto p_2 = make_pair(2, c);

        if (st.count(p_0))
        {
            st.erase(p_0);
            st.insert(p_1);
            c_to_t[c].push_back(i);
        }
        else if (st.count(p_1))
        {
            st.erase(p_1);
            st.insert(p_2);
            c_to_t[c].push_back(i);
    ...
    while (1)
    {
        ...
        if (num == 2)
        {
            ...
            if (depth[j1] < k[j1])
            {
                int c_1 = a[j1][depth[j1]];
                auto p_0_j1 = make_pair(0, c_1);
                auto p_1_j1 = make_pair(1, c_1);
                auto p_2_j1 = make_pair(2, c_1);

                if (st.count(p_0_j1))
                {
                    st.erase(p_0_j1);
                    st.insert(p_1_j1);
                    c_to_t[c_1].push_back(j1);
                }
                else if (st.count(p_1_j1))
                {
                    st.erase(p_1_j1);
                    st.insert(p_2_j1);
                    c_to_t[c_1].push_back(j1);
            ...
            if (depth[j2] < k[j2])
            {
                int c_2 = a[j2][depth[j2]];
                auto p_0_j2 = make_pair(0, c_2);
                auto p_1_j2 = make_pair(1, c_2);
                auto p_2_j2 = make_pair(2, c_2);

                if (st.count(p_0_j2))
                {
                    st.erase(p_0_j2);
                    st.insert(p_1_j2);
                    c_to_t[c_2].push_back(j2);
                }
                else if (st.count(p_1_j2))
                {
                    st.erase(p_1_j2);
                    st.insert(p_2_j2);
                    c_to_t[c_2].push_back(j2);
