...
int main()
{
    ...
    int BIT = (1 << N);
    vector<vector<long long>> dp(BIT, vector<long long>(M + 1, INF_ll));
    for (int j = 0; j <= M; j++)
    {
        dp[0][j] = 0;
    }
    ...
    for (int j = 0; j < M; j++)
    {
        ...
        for (int bit = 0; bit < BIT; bit++)
        {
            if (dp[bit][j] != INF_ll)
            {
                int next_bit = bit | c[j + 1];

                if (next_bit < BIT)
                {
                    chmin(dp[next_bit][j + 1], dp[bit][j] + a[j + 1]);
                }
            }
        }
    }

    if (dp[BIT - 1][M] == INF_ll)
    {
        cout << -1 << endl;
    }
    else
    {
        cout << dp[BIT - 1][M] << endl;
    }
}