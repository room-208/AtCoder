...
bool judge(double r, double c_x, double c_y, double x, double y)
{
    double t_x = x - c_x;
    double t_y = y - c_y;

    if (t_x * t_x + t_y * t_y <= r * r)
    {
        return true;
    }
    else
        ...
        for (int i = 0; i < N; i++)
        {
            for (int j = i + 1; j < N; j++)
            {
                for (int k = j + 1; k < N; k++)
                {
                    ...
                    {
                        bool flag = true;
                        double c_y = ((e - a) * (a * a + b * b - c * c - d * d) - (c - a) * (a * a + b * b - e * e - f * f)) / (2. * (e - a) * (b - d) - 2. * (c - a) * (b - f));
                        double c_x = (2. * (b - d) * c_y - a * a - b * b + c * c + d * d) / (2. * (c - a));
                        double r = dist(x[i], y[i], c_x, c_y);

                        for (int l = 0; l < N; l++)
                        {
                            if (!judge(r, c_x, c_y, x[l], y[l]))
                            {
                                flag = false;
                                break;
                            }
