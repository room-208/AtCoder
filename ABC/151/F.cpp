...
double x_p(double x1, double y1, double x2, double y2, double x3, double y3) {
  double A = (y1 - y2) * (x3 * x3 - x1 * x1 + y3 * y3 - y1 * y1) -
             (y1 - y3) * (x2 * x2 - x1 * x1 + y2 * y2 - y1 * y1);
  double B = 2. * (x1 - x2) * (y1 - y3) - 2. * (x1 - x3) * (y1 - y2);
  return A / B;
}

double y_p(double x1, double y1, double x2, double y2, double x3, double y3) {
  double A = (x1 - x3) * (x2 * x2 - x1 * x1 + y2 * y2 - y1 * y1) -
             (x1 - x2) * (x3 * x3 - x1 * x1 + y3 * y3 - y1 * y1);
  double B = 2. * (x1 - x2) * (y1 - y3) - 2. * (x1 - x3) * (y1 - y2);
  return A / B;
}

bool IsIn(vector<double> x, vector<double> y, double xp, double yp, double r) {
  bool flag = true;
  for (int i = 0; i < (int)x.size(); i++) {
    if ((x[i] - xp) * (x[i] - xp) + (y[i] - yp) * (y[i] - yp) > r * r) {
      flag = false;
    }
  }
  return flag;
...
int main() {
  ...
  for (int i = 0; i < N; i++) {
    for (int j = i + 1; j < N; j++) {
      ...
      double dx = x[i] - xp;
      double dy = y[i] - yp;
      double r = sqrt(dx * dx + dy * dy) + 1e-7;
      if (IsIn(x, y, xp, yp, r)) {
        chmin(ans, r);
      }
    }
    ...
    for (int j = i + 1; j < N; j++) {
      for (int k = j + 1; k < N; k++) {
        if ((y[k] - y[i]) * (x[j] - x[i]) == (y[j] - y[i]) * (x[k] - x[i])) {
          ...
          double xp = x_p(x[i], y[i], x[j], y[j], x[k], y[k]);
          double yp = y_p(x[i], y[i], x[j], y[j], x[k], y[k]);
          double dx = x[i] - xp;
          double dy = y[i] - yp;
          double r = sqrt(dx * dx + dy * dy) + 1e-7;
          if (IsIn(x, y, xp, yp, r)) {
