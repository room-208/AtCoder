...
int main()
{
    ...
    for (int i = 0; i < 3; i++)
    {
        x[i] = ceil((long double)A[i] / (long double)Y);
        S += x[i];
    }
    if (S <= X)
    {
    ...
    for (int i = 0; i < 3; i++)
    {
        y[i] = ceil((long double)A[i] / (long double)X);
        S += y[i];
    }
    if (S <= Y)
    {
    ...
    {
        long long X_tmp = X;
        long long x = ceil((long double)A[i] / (long double)Y);
        X_tmp -= x;
        ...
        for (int j = 0; j < 3; j++)
        {
            y[j] = ceil((long double)A[j] / (long double)X_tmp);
        }
    ...
    {
        long long Y_tmp = Y;
        long long y = ceil((long double)A[i] / (long double)X);
        Y_tmp -= y;
        ...
        for (int j = 0; j < 3; j++)
        {
            x[j] = ceil((long double)A[j] / (long double)Y_tmp);
        }
