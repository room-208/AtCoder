...
bool IsNan(vector<int> &dist, int K) {
  int num = -INF_int;
...
int main() {
  ...
  for (int i = 0; i < N; i++) {
    if (S[i] == 'o') {
      int n = upper_bound(A.begin(), A.end(), i + C) - A.begin();
  ...
  for (int i = N - 1; i >= 0; i--) {
    if (S[i] == 'o') {
      int n = lower_bound(A.begin(), A.end(), i - C) - A.begin();

      if (n != 0) {
        int a = A[n - 1];
        if (!binary_search(G[a].begin(), G[a].end(), i)) {
          G[a].push_back(i);
        }
      }
    }
