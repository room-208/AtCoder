...
int main()
{
    ...
    vector<vector<long long>> w(R, vector<long long>(R));
    for (int i = 0; i < R; i++)
    {
        vector<long long> d = Dijkstra(G, r[i]);
    ...
    vector<vector<long long>> dp((1 << R), vector<long long>(R, INF_ll));
    for (int i = 0; i < R; i++)
    {
        dp[(1 << i)][i] = 0;
    }
