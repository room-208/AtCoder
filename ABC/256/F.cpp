...
mint GetAns(int x, Segtree &sgt0, Segtree &sgt1, Segtree &sgt2) {
  mint sum0 = sgt0.prod(1, x + 1);
  mint sum1 = sgt1.prod(1, x + 1);
  mint sum2 = sgt2.prod(1, x + 1);
  mint X = x;
  sum0 *= (X * X + X * 3 + 2);
  sum1 *= (X * 2 + 3);
  mint res = sum0 - sum1 + sum2;
  res /= 2;
  return res;
}
...
int main() {
  ...
  Segtree sgt0(A0), sgt1(A1), sgt2(A2);
  for (int i = 0; i < Q; i++) {
    if (t[i] == 1) {
      sgt0.set(x[i], v[i]);
      sgt1.set(x[i], v[i] * x[i]);
      sgt2.set(x[i], v[i] * x[i] * x[i]);
    } else {
      cout << GetAns(x[i], sgt0, sgt1, sgt2).val() << endl;
    }
  }
}