...
int main()
{
    ...
    vector<vector<vector<tuple<long long, long long, long long, long long>>>> dp(M + 1, vector<vector<tuple<long long, long long, long long, long long>>>(N + 1, vector<tuple<long long, long long, long long, long long>>(8, make_tuple(-INF_ll, 0, 0, 0))));
    vector<int> sign(3, 1);
    for (int bit = 0; bit < (1 << 3); bit++)
    {
        ...
        dp[0][0][bit] = make_tuple(0, 0, 0, 0);

        for (int j = 1; j <= N; j++)
        {
            for (int i = 0; i <= M; i++)
            {
                dp[i][j][bit] = dp[i][j - 1][bit];
            }

            for (int i = 0; i < M; i++)
            {
                long long J = get<0>(dp[i][j - 1][bit]);

                if (J != -INF_ll)
                {
                    long long X = get<1>(dp[i][j - 1][bit]);
                    long long Y = get<2>(dp[i][j - 1][bit]);
                    long long Z = get<3>(dp[i][j - 1][bit]);

                    long long X_next = X + x[j];
                    long long Y_next = Y + y[j];
                    long long Z_next = Z + z[j];
                    long long J_next = sign[0] * X_next + sign[1] * Y_next + sign[2] * Z_next;

                    if (J_next > get<0>(dp[i + 1][j][bit]))
                    {
                        dp[i + 1][j][bit] = make_tuple(J_next, X_next, Y_next, Z_next);
                    }
                }
            }
        }
