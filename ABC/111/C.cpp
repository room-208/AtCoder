...
int main()
{
    ...
    vector<vector<my_struct>> s(2, vector<my_struct>(M));
    for (int k = 0; k < 2; k++)
    {
        for (int i = 0; i < M; i++)
        {
            s[k][i].score = S[k] - cnt[i][k];
            s[k][i].num = i;
        }
    }
    ...
    int ans = INF_int;
    if (s[0][0].num != s[1][0].num)
    {
        chmin(ans, s[0][0].score + s[1][0].score);
    }
    chmin(ans, s[0][1].score + s[1][0].score);
    chmin(ans, s[0][0].score + s[1][1].score);
