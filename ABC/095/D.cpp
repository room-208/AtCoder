...
int main()
{
    ...
    vector<vector<long long>> S(N, vector<long long>(2, 0));
    for (int i = 0; i < N; i++)
    {
        if (i == 0)
        {
    ...
    vector<vector<long long>> V_max(N, vector<long long>(2, -INF_ll));
    for (int i = 0; i < N; i++)
    {
        if (i == 0)
        {
        ...
        {
            V_max[i][0] = V_max[i - 1][0];
            chmax(V_max[i][0], V[i][0]);
        }
    }
    for (int i = N - 1; i >= 0; i--)
    {
        ...
        {
            V_max[i][1] = V_max[i + 1][1];
            chmax(V_max[i][1], V[i][1]);
        }
    }

    long long ans = -INF_ll;
    chmax(ans, V_max[N - 1][0]);
    chmax(ans, V_max[0][1]);
    for (int i = 0; i < N - 1; i++)
    {
        chmax(ans, V[i][0] - x[i] + V_max[i + 1][1]);
    }
    for (int i = N - 1; i >= 1; i--)
    {
        chmax(ans, V[i][1] - (C - x[i]) + V_max[i - 1][0]);
    }
