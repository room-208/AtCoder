...
//等差数列の和
mint tousa_sum(long long A, long long D, long long N) {
  mint a = A;
  mint d = D;
  mint n = N;
  return (a * mint(2) + d * (n - 1)) * n / mint(2);
}
...
int main() {
  ...
  mint ans = 0;
  for (int k = 0; k <= K - 1; k++) {
    long long sum = pow(10LL, (long long)k + 1LL) - pow(10LL, (long long)k);
    ans += tousa_sum(1, 1, sum);
  }
  long long a = pow(10LL, (long long)K);
  long long sum_new = N - a + 1;
  ans += tousa_sum(1, 1, sum_new);
