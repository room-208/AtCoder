...
int main()
{
    ...
    unordered_multimap<string, int> ump_front;
    for (int i = 0; i < N; i++)
    {
        string front = s[i].substr(0, 3);
        ump_front.insert(make_pair(front, i));
    }

    unordered_multimap<string, int> ump_end;
    for (int i = 0; i < N; i++)
    {
        int size = s[i].size();
        string end = s[i].substr(size - 3, 3);
        ump_end.insert(make_pair(end, i));
    }

    /*
    for (auto itr = ump_front.begin(); itr != ump_front.end(); itr++)
    {
        cout << itr->first << " " << itr->second << endl;
    }
    for (auto itr = ump_end.begin(); itr != ump_end.end(); itr++)
    {
        cout << itr->first << " " << itr->second << endl;
    }
    */
    ...
    Graph_Edge G(N);
    for (auto itr_f = ump_front.begin(); itr_f != ump_front.end(); itr_f++)
    {
        string front = itr_f->first;
        auto itr_e = ump_end.find(front);

        if (itr_e != ump_end.end())
        {
            while (itr_e->first == front)
            {
                int a, b, w;
                a = itr_e->second;
                b = itr_f->second;
