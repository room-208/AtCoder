...
// 深さ優先探索
void DFS(int i, int j, int n, int s, int H, int W, vector<int> &a, vector<vector<bool>> &seen, vector<vector<int>> &c, int &cnt)
{
    seen[i][j] = true;
    c[i][j] = n;
    a[n]--;
    ...
    if (s == 0)
    {
        ...
        {
            s = 1;
            DFS(i + 1, j, n, s, H, W, a, seen, c, cnt);
        }
        else
        {
            DFS(i, j + 1, n, s, H, W, a, seen, c, cnt);
        }
    }
    else if (s == 1)
    {
