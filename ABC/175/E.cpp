...
int main()
{
    ...
    for (int k = 0; k < K; k++)
    {
        mp[make_pair(r[k], c[k])] = v[k];
    }

    vector<vector<vector<long long>>> dp(R, vector<vector<long long>>(C, vector<long long>(4, -1)));
    dp[0][0][0] = 0;
    ...
    for (int i = 0; i < R; i++)
    {
        for (int j = 0; j < C; j++)
        {
            ...
            if (in_out(p, q, R, C))
            {
                ...
                {
                    long long value = mp[make_pair(p, q)];

                    //0個
                    chmax(dp[p][q][0], dp[i][j][0]);

                    //1個
                    chmax(dp[p][q][1], dp[i][j][1]);
                    chmax(dp[p][q][1], dp[i][j][0] + value);

                    //2個
                    chmax(dp[p][q][2], dp[i][j][2]);
                    chmax(dp[p][q][2], dp[i][j][1] + value);

                    //3個
                    chmax(dp[p][q][3], dp[i][j][3]);
                    chmax(dp[p][q][3], dp[i][j][2] + value);
                }
                else
                {
                    chmax(dp[p][q][0], dp[i][j][0]);
        ...
        for (int j = 0; j < C; j++)
        {
            ...
            if (in_out(p, q, R, C))
            {
                if (mp.count(make_pair(p, q)))
                {
                    ...
                    //0個
                    chmax(dp[p][q][0], dp[i][j][3]);

                    //1個
                    chmax(dp[p][q][1], dp[i][j][3]);
                    chmax(dp[p][q][1], dp[i][j][3] + value);

                    //2個
                    chmax(dp[p][q][2], dp[i][j][3]);
                    chmax(dp[p][q][2], dp[i][j][3] + value);

                    //3個
                    chmax(dp[p][q][3], dp[i][j][3]);
                    chmax(dp[p][q][3], dp[i][j][3] + value);
                }
                else
                {
                    chmax(dp[p][q][0], dp[i][j][3]);
