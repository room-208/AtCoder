...
long long e_max() { return -INF_ll; }
long long op_min(long long a, long long b) { return min(a, b); }
long long e_min() { return INF_ll; }
}  // namespace seg
...
int main() {
  ...
  segtree<long long, seg::op_max, seg::e_max> sgt_max(y);
  segtree<long long, seg::op_min, seg::e_min> sgt_min(y);
  ...
  while (right - left > 1) {
    ...
    bool flag = false;
    for (int i = 0; i < N; i++) {
      long long key = x[i] + mid;
      int n = lower_bound(x.begin(), x.end(), key) - x.begin();
      if (n == N) {
        continue;
      }
      long long y_max = sgt_max.prod(n, N);
      long long y_min = sgt_min.prod(n, N);
      if (y_max - y[i] >= mid) {
        flag = true;
        break;
      }
