...
int main() {
  ...
  vector<vector<ll>> dist(H, vector<ll>(W, INF_ll));
  queue<pair<int, int>> que_play, que_magma;
  ...
  while (1) {
    ...
    if (t % K == 0) {
      ...
      while (!que_magma.empty()) {
        auto m = que_magma.front();
        que_magma.pop();
        for (int k = 0; k < 4; k++) {
          int ni = m.first + dx[k];
          int nj = m.second + dy[k];
          if (Is_in(ni, nj, H, W)) {
            if (A[ni][nj] == '.') {
              A[ni][nj] = '@';
              tmp.push({ni, nj});
    ...
    queue<pair<int, int>> tmp;
    while (!que_play.empty()) {
      auto p = que_play.front();
      que_play.pop();
