...
int main() {
  ...
  segtree<ll, seg::op_max, seg::e_max> sgt_A(A);
  segtree<ll, seg::op_min, seg::e_min> sgt_B(B);

  ll ans = 2LL * INF_ll;
  for (int i = 0; i < N; i++) {
    seg::target = A[i];
    int l = sgt_A.min_left<seg::f>(i + 1);
    int r = sgt_A.max_right<seg::f>(i);
    ll tmp = A[i] * sgt_B.prod(l, r);
    chmin(ans, tmp);
    chmin(ans, A[i] * B[i]);
  }
