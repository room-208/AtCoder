...
Graph_int GetGraph(int H, int W, vector<string> G) {
  Graph_int g(H * W);
  for (int i = 0; i < H; i++) {
    for (int j = 0; j < W; j++) {
      if (G[i][j] == '#') {
...
vector<vector<ll>> GetDist(Graph_int g, int N, int W, vector<int> C,
                           vector<int> D) {
  vector<vector<ll>> dist(N, vector<ll>(N, INF_ll));
  for (int i = 0; i < N; i++) {
    int s = to_node(C[i], D[i], W);
    auto d = BFS(g, s);
    for (int j = 0; j < N; j++) {
      int t = to_node(C[j], D[j], W);
      if (d[t] == -1) {
        dist[i][j] = INF_ll;
      } else {
        dist[i][j] = d[t];
...
ll update(ll d, int i, vector<ll> &S, vector<ll> &T) {
  if (T[i] < d) {
    return INF_ll;
  }
...
void CalInit(Graph_int g, int W, vector<vector<ll>> &dp, vector<int> C,
             vector<int> D, vector<ll> S, vector<ll> T) {
  int N = C.size();
  auto d = BFS(g, 0);

  for (int i = 0; i < N; i++) {
    int bit = (1 << i);
    int t = to_node(C[i], D[i], W);
    if (d[t] == -1) {
      dp[bit][i] = INF_ll;
    } else {
      dp[bit][i] = update(d[t], i, S, T);
    }
  }
}
...
int main() {
  ...
  auto g = GetGraph(H, W, G);
  auto dist = GetDist(g, N, W, C, D);

  vector<vector<ll>> dp((1 << N), vector<ll>(N, INF_ll));
  CalInit(g, W, dp, C, D, S, T);
    ...
    for (int v = 0; v < N; v++) {
      if (bit & (1 << v)) {
        for (int u = 0; u < N; u++) {
          ...
          if (dp[bit & ~(1 << v)][u] == INF_ll) {
            continue;
          }
          ll tmp = update(dp[bit & ~(1 << v)][u] + dist[u][v], v, S, T);
          chmin(dp[bit][v], tmp);
        }
      }
    }
