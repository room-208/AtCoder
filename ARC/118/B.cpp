...
int main()
{
    ...
    while (right - left > 1.)
    {
        ...
        {
            long double zero = 0.;
            long double r = (M * A[i] + mid) / N;
            long double l = max(zero, (M * A[i] - mid) / N);
            r = floor(r);
            l = ceil(l);
    ...
    vector<long double> R(K), L(K);
    for (int i = 0; i < K; i++)
    {
        long double zero = 0.;
        long double r = (M * A[i] + right) / N;
        long double l = max(zero, (M * A[i] - right) / N);
        r = floor(r);
        l = ceil(l);
