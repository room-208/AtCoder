...
int main()
{
    ...
    for (int i = 0; i < N; i++)
    {
        if (w[i] == WEIGHT[0])
        {
            VALUE[0].push_back(v[i]);
        }
        else if (w[i] == WEIGHT[1])
        {
            VALUE[1].push_back(v[i]);
        }
        else if (w[i] == WEIGHT[2])
        {
            VALUE[2].push_back(v[i]);
        }
        else if (w[i] == WEIGHT[3])
        {
            VALUE[3].push_back(v[i]);
        }
    }

    vector<unordered_map<long long, long long>> mp(4);
    mp[0][0] = 0;
    mp[1][0] = 0;
    mp[2][0] = 0;
    mp[3][0] = 0;
    ...
    for (int n = 0; n < 4; n++)
    {
        unordered_map<long long, long long> mp_tmp;

        for (auto value : VALUE[n])
        {
            mp_tmp = mp[n];

            for (auto p : mp[n])
            {
                if (mp_tmp.count(p.first + WEIGHT[n]))
                {
                    chmax(mp_tmp[p.first + WEIGHT[n]], p.second + value);
                }
                else
                {
                    mp_tmp[p.first + WEIGHT[n]] = p.second + value;
                }
            }
    ...
    for (auto p0 : mp[0])
    {
        for (auto p1 : mp[1])
        {
            for (auto p2 : mp[2])
            {
                for (auto p3 : mp[3])
                {
                    long long weight = p0.first + p1.first + p2.first + p3.first;
                    long long value = p0.second + p1.second + p2.second + p3.second;
