...
int main()
{
    ...
    vector<vector<long double>> dp(2, vector<long double>(N + 1, -1.));
    vector<vector<tuple<int, int, int>>> p(2, vector<tuple<int, int, int>>(N + 1));
    dp[0][0] = 0.;
    dp[1][0] = -10000000.;
    p[0][0] = make_tuple(-1, -1, -1);
    p[1][0] = make_tuple(-1, -1, -1);
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        p[0][i + 1] = make_tuple(0, i, 0);
        p[1][i + 1] = make_tuple(1, i, 0);

        if (chmax(dp[0][i + 1], dp[1][i] - A[i]))
        {
            p[0][i + 1] = make_tuple(1, i, 1);
        }
        if (chmax(dp[1][i + 1], dp[0][i] + A[i]))
        {
            p[1][i + 1] = make_tuple(0, i, 1);
        }
    }

    vector<int> ans;
    tuple<int, int, int> q = p[0][N];
    while (q != make_tuple(-1, -1, -1))
    {
        int f = get<0>(q);
        int i = get<1>(q);
        int tmp = get<2>(q);
