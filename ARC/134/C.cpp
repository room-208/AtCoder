...
bool IsOK(int N, int K, vector<long long> &a) {
  long long sum = 0;
  for (int i = 1; i < N; i++) {
    sum += a[i];
  }
...
long long getA(int N, int K, vector<long long> &a) {
  long long sum = 0;
  for (int i = 1; i < N; i++) {
    sum += a[i];
  }
...
mint com(long long n, long long r, vector<mint> &inv) {
  mint res = 1;
  for (int i = 0; i < r; i++) {
    res *= (n - i);
    res *= inv[i + 1];
...
int main() {
  ...
  vector<mint> inv(1000);
  for (int i = 1; i < 1000; i++) {
    inv[i] = mint(i).inv();
  }

  mint ans = 1;
  ans *= com(A + K - 1, K - 1, inv);
  for (int i = 1; i < N; i++) {
    ans *= com(a[i] + K - 1, K - 1, inv);
  }
