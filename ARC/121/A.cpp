...
int main()
{
    ...
    long long x_max = -INF_ll;
    int i_x_max;
    long long x_min = INF_ll;
    int i_x_min;
    long long y_max = -INF_ll;
    int i_y_max;
    long long y_min = INF_ll;
    int i_y_min;
    ...
    for (int i = 0; i < N; i++)
    {
        st.insert(make_pair(i, i_x_max));
        st.insert(make_pair(i, i_x_min));
        st.insert(make_pair(i, i_y_max));
        st.insert(make_pair(i, i_y_min));
    }
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        if (st.count(make_pair(i, i_x_max)))
        {
            if (chmax(tmp, x_max - x[i]))
            {
                j = i_x_max;
            }
        }
        if (st.count(make_pair(i, i_x_min)))
        {
            if (chmax(tmp, x[i] - x_min))
            {
                j = i_x_min;
            }
        }
        if (st.count(make_pair(i, i_y_max)))
        {
            if (chmax(tmp, y_max - y[i]))
            {
                j = i_y_max;
            }
        }
        if (st.count(make_pair(i, i_y_min)))
        {
            if (chmax(tmp, y[i] - y_min))
            {
                j = i_y_min;
            }
        }
