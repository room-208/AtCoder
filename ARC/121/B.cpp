...
int main()
{
    ...
    if (mp['R'] % 2 == 0 && mp['G'] % 2 == 0 && mp['B'] % 2 == 0)
    {
        cout << 0 << endl;
        return 0;
    }

    vector<long long> odd_1, odd_2, even;
    ...
    // odd_1,odd_2
    for (int i = 0; i < (int)odd_1.size(); i++)
    {
        long long key = odd_1[i];
        int m = lower_bound(odd_2.begin(), odd_2.end(), key) - odd_2.begin();

        if (m == (int)odd_2.size())
        {
            chmin(ans, llabs(key - odd_2[m - 1]));
        }
        else if (m == 0)
        {
            chmin(ans, llabs(key - odd_2[m]));
        }
        else
        {
            chmin(ans, llabs(key - odd_2[m]));
    ...
    for (int i = 0; i < (int)odd_2.size(); i++)
    {
        long long key = odd_2[i];
        int m = lower_bound(odd_1.begin(), odd_1.end(), key) - odd_1.begin();

        if (m == (int)odd_1.size())
        {
            chmin(ans, llabs(key - odd_1[m - 1]));
        }
        else if (m == 0)
        {
            chmin(ans, llabs(key - odd_1[m]));
        }
        else
        {
            chmin(ans, llabs(key - odd_1[m]));
    ...
    // odd_1,odd_2,even
    long long tmp_1 = INF_ll;
    long long tmp_2 = INF_ll;
    for (int i = 0; i < (int)even.size(); i++)
    {
        long long key = even[i];

        int m = lower_bound(odd_1.begin(), odd_1.end(), key) - odd_1.begin();

        if (m == (int)odd_1.size())
        {
            chmin(tmp_1, llabs(key - odd_1[m - 1]));
        }
        else if (m == 0)
        {
            chmin(tmp_1, llabs(key - odd_1[m]));
        }
        else
        {
            chmin(tmp_1, llabs(key - odd_1[m]));
    ...
    for (int i = 0; i < (int)even.size(); i++)
    {
        ...
        if (m == (int)odd_2.size())
        {
            chmin(tmp_2, llabs(key - odd_2[m - 1]));
        }
        else if (m == 0)
        {
            chmin(tmp_2, llabs(key - odd_2[m]));
        }
        else
        {
            chmin(tmp_2, llabs(key - odd_2[m]));
