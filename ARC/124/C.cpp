...
int main()
{
    ...
    unordered_set<long long> st_tmp;
    for (auto s : st)
    {
        bool flag = true;
    ...
    for (int i = 0; i < (int)c.size(); i++)
    {
        for (int j = 0; j < (int)d.size(); j++)
        {
            ...
            for (int k = 0; k < N; k++)
            {
                if (a[k] % c[i] == 0 && b[k] % d[j] == 0)
                {
                    continue;
                }
                else if (a[k] % d[j] == 0 && b[k] % c[i] == 0)
                {
                    continue;
                }
                else
            ...
            if (flag)
            {
                long long tmp = (c[i] / GCD(c[i], d[j])) * d[j];
                chmax(ans, tmp);
            }
        }
    }
