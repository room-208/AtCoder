...
void DFS(const Graph_int &G, int v, vector<int> &a, unordered_map<pair<int, int>, int, HashPair> &mp, int N, vector<bool> &seen, int p = -1)
{
    seen[v] = true;

    if (p != -1)
    {
        int c = mp[make_pair(v, p)];
...
int main()
{
    ...
    for (int i = 0; i < M; i++)
    {
        mp[make_pair(u[i], v[i])] = c[i];
        mp[make_pair(v[i], u[i])] = c[i];
    }

    Graph_int G(N);
    for (auto itr = mp.begin(); itr != mp.end(); itr++)
    {
        int p = itr->first.first;
        int q = itr->first.second;
        G[p].push_back(q);
        G[q].push_back(p);
    }
