...
int main() {
  ...
  for (int i = 0; i < M; i++) {
    ...
    u--;
    v--;
    G_y[u].push_back(Edge(v, a));
    G_y[v].push_back(Edge(u, a));
    G_s[u].push_back(Edge(v, b));
    G_s[v].push_back(Edge(u, b));
  }

  auto dy = Dijkstra(G_y, s);
  auto ds = Dijkstra(G_s, t);

  vector<long long> res(N, 1000000000000000);
  for (int i = 0; i < N; i++) {
    res[i] -= dy[i] + ds[i];
  }
