...
int main()
{
    ...
    */
    ...
    //交差点の価値
    vector<vector<double>> value(N + 2, vector<double>(N + 2, 0.));
    for (auto itr = cross.begin(); itr != cross.end(); itr++)
    {
        int i = itr->first;
        int j = itr->second;
        ...
        //上
        for (int tmp_i = i + 1; tmp_i <= N + 1; tmp_i++)
        {
            if (c[tmp_i][j] != '#')
            {
                cnt++;
        ...
        //右
        for (int tmp_j = j + 1; tmp_j <= N + 1; tmp_j++)
        {
            if (c[i][tmp_j] != '#')
            {
                cnt++;
        ...
        //左
        for (int tmp_j = j - 1; tmp_j >= 0; tmp_j--)
        {
            if (c[i][tmp_j] != '#')
            {
                cnt++;
