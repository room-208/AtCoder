...
long long CalcDissatisfy(int d, vector<long long> &c, vector<int> &last)
{
    long long res = 0;
...
vector<long long> CalcScore(vector<long long> &c, vector<vector<long long>> &s, vector<int> &t)
{
    int D = (int)t.size();
    ...
    for (int d = 1; d <= D; d++)
    {
        res[d] = res[d - 1] + s[d - 1][t[d - 1]];

        last[t[d - 1]] = d;

        res[d] -= CalcDissatisfy(d, c, last);
    }
...
vector<int> CalcGreedyScore(int D, vector<long long> &c, vector<vector<long long>> &s)
{
    vector<int> res(D);
    vector<int> last(26, 0);
    ...
    for (int d = 1; d <= D; d++)
    {
        ...
        for (int i = 0; i < 26; i++)
        {
            long long tmp = s[d - 1][i] - CalcDissatisfy(d, c, last);
...
int main()
{
    ...
    vector<int> t = CalcGreedyScore(D, c, s);
    for (int i = 0; i < (int)t.size(); i++)
    {
        cout << t[i] + 1 << endl;
    }

    // cout << CalcScore(c, s, t).back() << endl;
}