...
long long ChangeFinalScore(int d, int q, vector<long long> &c, vector<vector<long long>> &s, vector<int> &t)
{
    //更新
    t[d - 1] = q;

    vector<long long> res = CalcScore(c, s, t);
...
int main()
{
    ...
    for (int i = 0; i < M; i++)
    {
        cout << ChangeFinalScore(d[i], q[i], c, s, t) << endl;
    }
}