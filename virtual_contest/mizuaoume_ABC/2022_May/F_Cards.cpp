...
int main() {
  ...
  for (int i = 0; i < N; i++) {
    if (i == uf.root(i)) {
      int n = uf.size(i);
      vector<vector<mint>> dp0(n, vector<mint>(2, 0));
      dp0[0][0] = 1;
      for (int j = 0; j < n - 1; j++) {
        dp0[j + 1][0] += dp0[j][1];
        dp0[j + 1][1] += dp0[j][0] + dp0[j][1];
      }
      vector<vector<mint>> dp1(n, vector<mint>(2, 0));
      dp1[0][1] = 1;
      for (int j = 0; j < n - 1; j++) {
        dp1[j + 1][0] += dp1[j][1];
        dp1[j + 1][1] += dp1[j][0] + dp1[j][1];
      }
      ans *= dp0[n - 1][1] + dp1[n - 1][1] + dp1[n - 1][0];
    }
  }
