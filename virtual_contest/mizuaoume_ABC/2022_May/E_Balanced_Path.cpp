...
int main() {
  ...
  for (int i = 0; i < H; i++) {
    for (int j = 0; j < W; j++) {
      C[i][j] = llabs(A[i][j] - B[i][j]);
    }
  }

  int VAL = 80 * 160;
  vector<vector<vector<bool>>> dp(
      H, vector<vector<bool>>(W, vector<bool>(VAL + 1, false)));
  dp[0][0][C[0][0]] = true;
  for (int i = 0; i < H; i++) {
    for (int j = 0; j < W; j++) {
      if (Is_in(i + 1, j, H, W)) {
        for (int val = 0; val <= VAL; val++) {
          if (dp[i][j][val]) {
            int next_val1 = abs(val + C[i + 1][j]);
            int next_val2 = abs(val - C[i + 1][j]);
            if (0 <= next_val1 && next_val1 <= VAL) {
              dp[i + 1][j][next_val1] = true;
            }
            if (0 <= next_val2 && next_val2 <= VAL) {
              dp[i + 1][j][next_val2] = true;
            }
          }
        }
      }
      if (Is_in(i, j + 1, H, W)) {
        for (int val = 0; val <= VAL; val++) {
          if (dp[i][j][val]) {
            int next_val1 = abs(val + C[i][j + 1]);
            int next_val2 = abs(val - C[i][j + 1]);
            if (0 <= next_val1 && next_val1 <= VAL) {
              dp[i][j + 1][next_val1] = true;
            }
            if (0 <= next_val2 && next_val2 <= VAL) {
              dp[i][j + 1][next_val2] = true;
            }
          }
        }
      }
