...
int main() {
  ...
  map<pair<int, int>, int> dp, dp_new;
  dp[{0, 0}] = 0;
  for (int i = 0; i < N; i++) {
    dp_new = dp;
    for (auto itr = dp.begin(); itr != dp.end(); itr++) {
      int A, B;
      tie(A, B) = itr->first;
      if (dp_new.count({A + a[i], B + b[i]})) {
        chmin(dp_new[{A + a[i], B + b[i]}], itr->second + c[i]);
      } else {
        dp_new[{A + a[i], B + b[i]}] = itr->second + c[i];
      }
    }
    dp = dp_new;
  }
  ...
  int ans = INF_int;
  for (int k = 1; k <= 40; k++) {
    if (dp.count({Ma * k, Mb * k})) {
      chmin(ans, dp[{Ma * k, Mb * k}]);
    }
  }
