...
double GetXp(double x1, double x2, double x3, double y1, double y2, double y3) {
  double bunshi = (y1 - y2) * (x3 * x3 - x1 * x1 + y3 * y3 - y1 * y1) -
                  (y1 - y3) * (x2 * x2 - x1 * x1 + y2 * y2 - y1 * y1);
  double bunbo = 2. * (x1 - x2) * (y1 - y3) - 2. * (x1 - x3) * (y1 - y2);
  return bunshi / bunbo;
}

double GetYp(double x1, double x2, double x3, double y1, double y2, double y3) {
  double bunshi = (x1 - x3) * (x2 * x2 - x1 * x1 + y2 * y2 - y1 * y1) -
                  (x1 - x2) * (x3 * x3 - x1 * x1 + y3 * y3 - y1 * y1);
  double bunbo = 2. * (x1 - x2) * (y1 - y3) - 2. * (x1 - x3) * (y1 - y2);
  return bunshi / bunbo;
}

double GetR(double x1, double xp, double y1, double yp) {
  return sqrt((xp - x1) * (xp - x1) + (yp - y1) * (yp - y1));
}

bool Bug(double x1, double x2, double x3, double y1, double y2, double y3) {
...
bool Is_OK(vector<double> &x, vector<double> &y, double xp, double yp,
           double r) {
  int N = x.size();
  for (int i = 0; i < N; i++) {
    if ((x[i] - xp) * (x[i] - xp) + (y[i] - yp) * (y[i] - yp) > r * r + EPS) {
      return false;
    }
  }
  return true;
...
int main() {
  ...
  for (int i = 0; i < N; i++) {
    for (int j = i + 1; j < N; j++) {
      double xp = (x[i] + x[j]) / 2.;
      double yp = (y[i] + y[j]) / 2.;
      double r = GetR(x[i], xp, y[i], yp);
      if (Is_OK(x, y, xp, yp, r)) {
        chmin(ans, r);
      }
    }
  }
  ...
  for (int i = 0; i < N; i++) {
    for (int j = i + 1; j < N; j++) {
      for (int k = j + 1; k < N; k++) {
        double xp = GetXp(x[i], x[j], x[k], y[i], y[j], y[k]);
        double yp = GetYp(x[i], x[j], x[k], y[i], y[j], y[k]);
        double r = GetR(x[i], xp, y[i], yp);
        if (Bug(x[i], x[j], x[k], y[i], y[j], y[k])) {
          continue;
        }
        if (Is_OK(x, y, xp, yp, r)) {
          chmin(ans, r);
