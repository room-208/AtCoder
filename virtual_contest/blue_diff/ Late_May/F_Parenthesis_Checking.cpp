...
// セグメント木のエイリアス
using LazySegtreeMin = lazy_segtree<ll, seg::op, seg::e, ll, seg::mapping,
...
int main() {
  ...
  for (int i = 0; i < Q; i++) {
    if (t[i] == 1) {
      ...
      ll left_val = sgt.get(l[i]);
      ll right_val = sgt.get(r[i] + 1);
      ll min_val = sgt.prod(l[i], r[i] + 2);
      if (left_val == right_val && left_val <= min_val) {
        cout << "Yes" << endl;
      } else {
        cout << "No" << endl;
      }
