...
ll eval(ll a, const vector<ll> &x) {
  ll res = 0;
  for (auto &&b : x) {
    res += llabs(a - b);
  }
...
int main() {
  ...
  if (N % 2 == 1) {
    chmin(ans_x, eval(x[(N - 1) / 2], x));
    chmin(ans_y, eval(y[(N - 1) / 2], y));
  } else {
    chmin(ans_x, eval(x[(N / 2)], x));
    chmin(ans_y, eval(y[(N / 2)], y));
    chmin(ans_x, eval(x[(N / 2) - 1], x));
    chmin(ans_y, eval(y[(N / 2) - 1], y));
  }
