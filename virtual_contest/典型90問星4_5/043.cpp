...
class Rerooting {
 private:
  struct DP {
    ...
    ll subtree;
    DP() {}
    DP(ll dp, ll subtree) : dp(dp), subtree(subtree) {}
  };
...
// 01BFS
vector<ll> BFS_01(const Graph_Edge &G, vector<int> s) {
  int N = (int)G.size();       // 頂点数
...
int main() {
  ...
  for (int D = 0; D < 4; D++) {
    for (int i = 0; i < H; i++) {
      for (int j = 0; j < W; j++) {
        ...
        int from = to_node(i, j, W) + D * N;
        for (int k = 0; k < 4; k++) {
          int next_i = i + dx[k];
          int next_j = j + dy[k];
          if (in_out(next_i, next_j, H, W)) {
            int to = to_node(next_i, next_j, W) + k * N;
            if (k == D) {
  ...
  vector<int> t;
  for (int D = 0; D < 4; D++) {
    int node_s = to_node(rs, cs, W) + D * N;
    int node_t = to_node(rt, ct, W) + D * N;
    s.push_back(node_s);
    t.push_back(node_t);
  }

  auto dist = BFS_01(G, s);
