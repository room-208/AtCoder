...
int main() {
  ...
  while (right - left > 1) {
    ...
    for (int k = 0; k < K; k++) {
      B_min[k] = max(my_ceil(A[k] - mid, N), 0LL);
      B_max[k] = my_floor(A[k] + mid, N);
      if (B_min[k] > B_max[k]) {
        ok = false;
      }
    }
  ...
  for (int k = 0; k < K; k++) {
    B_min[k] = max(my_ceil(A[k] - right, N), 0LL);
    B_max[k] = my_floor(A[k] + right, N);
  }
