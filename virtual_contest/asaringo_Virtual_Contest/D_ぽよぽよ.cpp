...
vector<mint> GetS(ll l, vector<mint> dp) {
  vector<mint> S(2 * l + 2);
  S[0] = 0;
  for (int i = 0; i <= 2 * l; i++) {
    S[i + 1] = S[i] + dp[i];
...
int main() {
  ...
  vector<mint> dp(2 * l[0] + 1);
  for (int i = 0; i <= 2 * l[0]; i++) {
    dp[i] = 1;
  }

  for (int i = 1; i < N; i++) {
    vector<mint> S = GetS(l[i - 1], dp);
    vector<ll> x = Getx(p[i - 1], l[i - 1]);
    vector<ll> x_new = Getx(p[i], l[i]);
    vector<mint> dp_new(x_new.size(), 0);
    for (int j = 0; j < x_new.size(); j++) {
      int n = lower_bound(x.begin(), x.end(), x_new[j]) - x.begin();
      dp_new[j] = S[n] - S[0];
    }
    dp = dp_new;
  }
