...
ll GetNegativeCount(vector<ll> P, vector<ll> M, vector<ll> Z) {
  return (ll)(P.size()) * (ll)(M.size());
}

ll GetZeroCount(vector<ll> P, vector<ll> M, vector<ll> Z) {
  return (ll)(Z.size()) * (ll)(P.size() + M.size()) + COMLL(Z.size(), 2);
}

void Count(vector<ll> P, ll x, ll &cnt) {
  ll val = 0;
  for (int i = 0; i < P.size(); i++) {
    ll key = my_floor(x, P[i]);
    int n = upper_bound(P.begin(), P.end(), key) - P.begin();
    val += n;
    if (P[i] * P[i] <= x) {
      val--;
    }
...
int main() {
  ...
  if (K <= GetNegativeCount(P, M, Z)) {
    ...
    while (right - left > 1) {
      ...
      for (int i = 0; i < P.size(); i++) {
        ll key = my_floor(x, P[i]);
        int n = upper_bound(M.begin(), M.end(), key) - M.begin();
        cnt += n;
      }
      if (cnt >= K) {
        right = x;
  ...
  if (K <= GetNegativeCount(P, M, Z) + GetZeroCount(P, M, Z)) {
    cout << 0 << endl;
    return 0;
  }
  ...
  K -= GetNegativeCount(P, M, Z) + GetZeroCount(P, M, Z);
