...
int main() {
  ...
  vector<ll> ans(Q, INF_ll);
  for (int i = 0; i < Q; i++) {
    int ns = lower_bound(s.begin(), s.end(), x[i]) - s.begin();
    int nt = lower_bound(t.begin(), t.end(), x[i]) - t.begin();
    chmin(ans[i], llabs(s[ns] - x[i]) + llabs(t[nt] - s[ns]));
    chmin(ans[i], llabs(t[nt] - x[i]) + llabs(s[ns] - t[nt]));
    if (ns > 0) {
      chmin(ans[i], llabs(s[ns - 1] - x[i]) + llabs(t[nt] - s[ns - 1]));
      chmin(ans[i], llabs(t[nt] - x[i]) + llabs(s[ns - 1] - t[nt]));
    }
    if (nt > 0) {
      chmin(ans[i], llabs(s[ns] - x[i]) + llabs(t[nt - 1] - s[ns]));
      chmin(ans[i], llabs(t[nt - 1] - x[i]) + llabs(s[ns] - t[nt - 1]));
    }
    if (ns > 0 && nt > 0) {
      chmin(ans[i], llabs(s[ns - 1] - x[i]) + llabs(t[nt - 1] - s[ns - 1]));
      chmin(ans[i], llabs(t[nt - 1] - x[i]) + llabs(s[ns - 1] - t[nt - 1]));
    }
  }
