...
// 深さ優先探索
void DFS(const Graph_int &G, int v,
         unordered_map<int, unordered_map<int, int>> &mp, vector<int> &cnt,
         vector<int> &ans, int depth = 0, int p = -1) {
  if (mp.count(v)) {
    for (auto &&p : mp[v]) {
      ans[p.first] -= cnt[p.second];
    }
  }
  cnt[depth]++;
  for (auto next_v : G[v]) {
    ...
    DFS(G, next_v, mp, cnt, ans, depth + 1, v);
  }
  if (mp.count(v)) {
    for (auto &&p : mp[v]) {
      ans[p.first] += cnt[p.second];
