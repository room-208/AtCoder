...
// 深さ優先探索
void DFS(const Graph_int &G, vector<ll> &a, vector<ll> &l, vector<int> &ans,
         int v, int p = -1) {
  ll key = a[v];
  ll copy_val;
  int n = lower_bound(l.begin(), l.end(), key) - l.begin();
  if (n == l.size()) {
    l.push_back(key);
    chmax(ans[v], (int)l.size());
    for (auto next_v : G[v]) {
      if (next_v == p) {
        continue;
      }
      DFS(G, a, l, ans, next_v, v);
    }
    l.pop_back();
  } else {
    copy_val = l[n];
