...
ll f(ll t, ll D, ll C) { return t + C + my_floor(D, t + 1); }

ll GetMinTime(ll now, ll C, ll D) {
  ll res = f(now, D, C);
  ll t = my_sqrt(D) - 1;
  for (ll time = t - 10; time < t + 10; time++) {
    if (time >= now) {
      chmin(res, f(time, D, C));
    }
  }
  return res;
}
...
vector<ll> Dijkstra(const Graph_Edge &G, int s) {
  ...
  while (!que.empty()) {
    ...
    // 頂点 v を始点とした各辺を緩和
    for (auto e : G[v]) {
      ll next_time = GetMinTime(dist[v], e.c, e.d);
      if (chmin(dist[e.to], next_time)) {
        // 更新があるならヒープに新たに挿入
