...
tuple<vector<Ta>, vector<Tb>, vector<Tc>, vector<Td>> GetSortedTuple(
    vector<Ta> a, vector<Tb> b, vector<Tc> c, vector<Td> d,
    bool reverse_flag = false) {
  assert(a.size() == b.size());
  assert(b.size() == c.size());
  int N = a.size();
  vector<tuple<Ta, Tb, Tc, Td>> t(N);
  for (int i = 0; i < N; i++) {
    t[i] = {a[i], b[i], c[i], d[i]};
  }
  sort(t.begin(), t.end());
...
int main() {
  ...
  tie(C, A, B, index) = GetSortedTuple(C, A, B, index);
