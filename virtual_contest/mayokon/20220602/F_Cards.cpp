...
mint solveDP(int m) {
  ...
  vector<vector<mint>> dp0(m, vector<mint>(2, 0));
  dp0[0][0] = 1;
  for (int i = 1; i < m; i++) {
    dp0[i][0] += dp0[i - 1][1];
    dp0[i][1] += dp0[i - 1][0] + dp0[i - 1][1];
  }
  res += dp0[m - 1][1];

  vector<vector<mint>> dp1(m, vector<mint>(2, 0));
  dp1[0][1] = 1;
  for (int i = 1; i < m; i++) {
    dp1[i][0] += dp1[i - 1][1];
    dp1[i][1] += dp1[i - 1][0] + dp1[i - 1][1];
  }
  res += dp1[m - 1][0] + dp1[m - 1][1];
