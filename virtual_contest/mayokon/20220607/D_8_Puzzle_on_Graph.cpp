...
void GetGraph(vector<int> u, vector<int> v, Graph_int &G,
              map<vector<int>, int> &mp) {
  int M = u.size();
  Graph_int g(9);
  for (int i = 0; i < M; i++) {
...
int main() {
  ...
  map<vector<int>, int> mp = GetMap();
  int s = mp[p];
  int t = mp[{0, 1, 2, 3, 4, 5, 6, 7, 8}];
