...
// 深さ優先探索
void DFS(const Graph_int &G, int v, vector<pair<int, int>> &T,
         vector<bool> &seen, int p = -1) {
  seen[v] = true;
  if (p != -1) {
    T.push_back({p, v});
  }
  for (auto next_v : G[v]) {
    ...
    DFS(G, next_v, T, seen, v);
  }
}

//幅優先探索
vector<int> BFS(const Graph_int &G, int s, vector<pair<int, int>> &T) {
  int N = (int)G.size();    // 頂点数
...
int main() {
  ...
  for (int i = 0; i < N - 1; i++) {
    cout << T2[i].first + 1 << " " << T2[i].second + 1 << endl;
  }
}