...
int main() {
  ...
  auto d_from_u = BFS(G, u);
  auto d_from_v = BFS(G, v);

  int max_dist = -1;
  for (int i = 0; i < N; i++) {
    if (d_from_v[i] > d_from_u[i]) {
      chmax(max_dist, d_from_v[i]);
    }
  }
  ...
  int ans = 0;
  for (auto &&f : cands) {
    int delta = d_from_v[f] - d_from_u[f];
    if (delta % 2 == 0) {
      chmax(ans, d_from_v[f] - 1);
    } else {
      chmax(ans, d_from_v[f] - 1);
    }
  }
