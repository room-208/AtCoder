...
int main() {
  ...
  vector<vector<mint>> SX(H + 1, vector<mint>(W + 1, 0));
  vector<vector<mint>> SY(H + 1, vector<mint>(W + 1, 0));
  vector<vector<mint>> SXY(H + 1, vector<mint>(W + 1, 0));
  dp[0][0] = 1;
  for (int i = 0; i < H; i++) {
    for (int j = 0; j < W; j++) {
      if (S[i][j] == '#') {
      ...
      SY[i + 1][j] = SY[i][j] + dp[i][j];
      SX[i][j + 1] = SX[i][j] + dp[i][j];
      SXY[i + 1][j + 1] = SXY[i][j] + dp[i][j];
      dp[i + 1][j] += SY[i + 1][j];
      dp[i][j + 1] += SX[i][j + 1];
      dp[i + 1][j + 1] += SXY[i + 1][j + 1];
    }
  }
