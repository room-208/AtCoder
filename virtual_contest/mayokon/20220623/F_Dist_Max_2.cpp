...
ll e_min() { return INF_ll; }
ll op_max(ll a, ll b) { return max(a, b); }
ll e_max() { return -INF_ll; }
}  // namespace seg

// セグメント木のエイリアス
using SegtreeMin = segtree<ll, seg::op_min, seg::e_min>;
using SegtreeMax = segtree<ll, seg::op_min, seg::e_min>;
...
int main() {
  ...
  SegtreeMax sgt_max(y);
  SegtreeMin sgt_min(y);
  ...
  while (right - left > 1) {
    ...
    for (int i = 0; i < N; i++) {
      ...
      auto itr_min = upper_bound(x.begin(), x.end(), x_min);
      auto itr_max = lower_bound(x.begin(), x.end(), x_max);

      if (itr_min != x.begin()) {
        int r = itr_min - x.begin();
        ll val_min = sgt_min.prod(0, r);
        ll val_max = sgt_max.prod(0, r);
        if (llabs(y[i] - val_max) >= mid) {
          ok = true;
          break;
        }
        if (llabs(y[i] - val_min) >= mid) {
          ok = true;
          break;
        }
      }
      if (itr_max != x.end()) {
        int l = itr_max - x.begin();
        ll val_min = sgt_min.prod(l, N);
        ll val_max = sgt_max.prod(l, N);
        if (llabs(y[i] - val_max) >= mid) {
          ok = true;
          break;
        }
