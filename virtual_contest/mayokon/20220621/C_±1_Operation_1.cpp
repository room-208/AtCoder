...
ll GoodNum(ll A, ll D, ll i) { return A + D * i; }
...
int main() {
  ...
  if (D > 0) {
    if (GoodNum(A, D, N - 1) <= X) {
      cout << llabs(GoodNum(A, D, N - 1) - X) << endl;
      return 0;
    }
    if (X <= GoodNum(A, D, 0)) {
      cout << llabs(GoodNum(A, D, 0) - X) << endl;
      return 0;
    }
    ll n = my_floor(X - A, D);
    cout << min(llabs(GoodNum(A, D, n) - X), llabs(GoodNum(A, D, n + 1) - X))
         << endl;
    return 0;
  }
