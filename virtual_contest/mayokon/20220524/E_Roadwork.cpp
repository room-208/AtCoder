...
int main() {
  ...
  tie(S, T, X) = GetSortedTuple(S, T, X);

  vector<ll> ans(Q);
  multiset<pair<ll, ll>> st_r_x;
  multiset<ll> st_x;
  st_r_x.insert({INF_ll, INF_ll});
  st_x.insert(INF_ll);
  ll x;
  int j = 0;
  for (int i = 0; i < Q; i++) {
    ...
    while (j < N && S[j] <= x) {
      st_x.insert(X[j]);
      st_r_x.insert({T[j], X[j]});
      j++;
    }
    while (st_r_x.begin()->first <= x) {
      ll key = st_r_x.begin()->second;
      st_x.erase(st_x.find(key));
      st_r_x.erase(st_r_x.begin());
    }
