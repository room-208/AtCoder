...
ll func(ll x, vector<ll> &a, vector<ll> &t) {
  int N = a.size();
  for (int i = 0; i < N; i++) {
    if (t[i] == 1) {
      x += a[i];
...
ll GetLeft(vector<ll> a, vector<ll> t) {
  ll left = -INF_ll;
  ll right = INF_ll;
  ll min_val = func(left, a, t);
  while (right - left > 1) {
    ll mid = (right + left) / 2;
    ll val = func(mid, a, t);
    if (val == min_val) {
...
ll GetRight(vector<ll> a, vector<ll> t) {
  ll left = -INF_ll;
  ll right = INF_ll;
  ll max_val = func(right, a, t);
  while (right - left > 1) {
    ll mid = (right + left) / 2;
    ll val = func(mid, a, t);
    if (val == max_val) {
...
int main() {
  ...
  ll val_l = func(xl, a, t);
  ll val_r = func(xr, a, t);

  for (int i = 0; i < Q; i++) {
    if (xl < x[i] && x[i] < xr) {
      cout << x[i] + (-xl - xr + val_l + val_r) / 2 << endl;
    } else if (x[i] <= xl) {
      cout << val_l << endl;
    } else {
      cout << val_r << endl;
