...
int GetLen(int bit, int N, vector<int> &l) {
  int len = 0;
  for (int i = 0; i < N; i++) {
    if (bit & (1 << i)) {
      len += l[i];
...
int main() {
  ...
  int ans = INF_int;
  for (int bit_A = 0; bit_A < (1 << N); bit_A++) {
    for (int bit_B = 0; bit_B < (1 << N); bit_B++) {
      for (int bit_C = 0; bit_C < (1 << N); bit_C++) {
        if (bit_A & bit_B) {
          continue;
        }
        if (bit_B & bit_C) {
        ...
        if (__builtin_popcount(bit_A) == 0) {
          continue;
        }
        if (__builtin_popcount(bit_B) == 0) {
          continue;
        }
        if (__builtin_popcount(bit_C) == 0) {
          continue;
        }

        int a = GetLen(bit_A, N, l);
        int b = GetLen(bit_B, N, l);
        int c = GetLen(bit_C, N, l);

        int ketugou = __builtin_popcount(bit_A) + __builtin_popcount(bit_B) +
                      __builtin_popcount(bit_C) - 3;
        int tmp = abs(a - A) + abs(b - B) + abs(c - C);
        chmin(ans, tmp + ketugou * 10);
      }
    }
  }
