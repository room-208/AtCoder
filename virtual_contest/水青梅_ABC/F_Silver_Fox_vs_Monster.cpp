...
int main() {
  ...
  lazy_segtree<ll, seg::op, seg::e, ll, seg::mapping, seg::composition, seg::id>
      sgt(H);
  ...
  for (int i = 0; i < N; i++) {
    ...
    ll key = X[i] + 2LL * D;
    int n = upper_bound(X.begin(), X.end(), key) - X.begin();
    n--;
    ll cnt = my_ceil(sgt.get(i), A);
    ans += cnt;
    sgt.apply(i, n + 1, -cnt * A);
  }
