...
// 深さ優先探索
void DFS(const Graph_int &G, int v, vector<bool> &seen, int &ans, int s,
         int p = -1, int depth = 0) {
  seen[v] = true;

  for (auto next_v : G[v]) {
    if (next_v == s && next_v != p) {
      chmax(ans, depth + 1);
    }
    if (seen[next_v]) {
      continue;
    }
    DFS(G, next_v, seen, ans, s, v, depth + 1);
  }
}
    ...
    for (int j = 0; j < W; j++) {
      ...
      for (int k = 0; k < 4; k++) {
        ...
        if (in_out(next_i, next_j, H, W)) {
          ...
          int to = to_node(next_i, next_j, W);
          G[from].push_back(to);
        }
      }
    }
