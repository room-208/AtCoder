...
int main() {
  ...
  for (int i = 0; i < Q; i++) {
    int n = lower_bound(s.begin(), s.end(), x[i]) - s.begin();
    int m = lower_bound(t.begin(), t.end(), x[i]) - t.begin();
    ...
    ll ans = INF_ll;
    chmin(ans, llabs(x[i] - s1) + llabs(s1 - t1));
    chmin(ans, llabs(x[i] - s1) + llabs(s1 - t2));
    chmin(ans, llabs(x[i] - s2) + llabs(s2 - t1));
    chmin(ans, llabs(x[i] - s2) + llabs(s2 - t2));
    chmin(ans, llabs(x[i] - t1) + llabs(t1 - s1));
    chmin(ans, llabs(x[i] - t1) + llabs(t1 - s2));
    chmin(ans, llabs(x[i] - t2) + llabs(t2 - s1));
    chmin(ans, llabs(x[i] - t2) + llabs(t2 - s2));
    cout << ans << endl;
  }
}