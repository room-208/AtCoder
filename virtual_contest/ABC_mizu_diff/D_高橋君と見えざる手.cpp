...
int main() {
  ...
  ll max_val = 0;
  segtree<ll, seg::op_max, seg::e_max> sgt_max(A);
  segtree<ll, seg::op_min, seg::e_min> sgt_min(A);
  for (int i = 0; i < N - 1; i++) {
    ll val = sgt_max.prod(i + 1, N) - A[i];
    chmax(max_val, val);
  }
