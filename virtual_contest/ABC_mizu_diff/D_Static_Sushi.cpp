...
vector<ll> GetAccumMax(vector<ll> x, vector<ll> v) {
  int N = x.size();
  vector<ll> S(N + 1, -INF_ll);
  ll y = 0;
  ll val = 0;
  S[0] = 0;
  for (int i = 0; i < N; i++) {
...
int main() {
  ...
  vector<ll> S_r = GetAccumMax(x, v);
  auto v_l = v;
  auto x_l = x;
  reverse(v_l.begin(), v_l.end());
  reverse(x_l.begin(), x_l.end());
  for (int i = 0; i < N; i++) {
    x_l[i] = C - x_l[i];
  }
  vector<ll> S_l = GetAccumMax(x_l, v_l);

  ll ans = 0;
  chmax(ans, S_r[N]);
  chmax(ans, S_l[N]);
  for (int i = 0; i < N; i++) {
    ll val = S_r[i + 1] - x[i] + S_l[N - i - 1];
    chmax(ans, val);
  }
  for (int i = 0; i < N; i++) {
    ll val = S_l[i + 1] - x_l[i] + S_r[N - i - 1];
    chmax(ans, val);
  }
