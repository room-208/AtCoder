...
int main() {
  ...
  for (int i = 0; i < N; i++) {
    ...
    vector<ll> dp_new(2, INF_ll);
    chmin(dp_new[0], dp[0] + llabs(ball[i].back() - x[0]) +
                         llabs(ball[i].back() - ball[i].front()));
    chmin(dp_new[0], dp[1] + llabs(ball[i].back() - x[1]) +
                         llabs(ball[i].back() - ball[i].front()));
    chmin(dp_new[1], dp[0] + llabs(ball[i].front() - x[0]) +
                         llabs(ball[i].front() - ball[i].back()));
    chmin(dp_new[1], dp[1] + llabs(ball[i].front() - x[1]) +
                         llabs(ball[i].front() - ball[i].back()));
    dp = dp_new;
    x[0] = ball[i].front();
    x[1] = ball[i].back();
  ...
  ll ans = INF_ll;
  chmin(ans, dp[0] + llabs(x[0]));
  chmin(ans, dp[1] + llabs(x[1]));
  cout << ans << endl;
}