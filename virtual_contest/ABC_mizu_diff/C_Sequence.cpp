...
int main() {
  ...
  ll cnt = 0;
  segtree<ll, seg::op, seg::e> sgt_p(A);
  if (sgt_p.get(0) <= 0) {
    cnt += 1 - sgt_p.get(0);
    sgt_p.set(0, 1);
  }
  for (int i = 2; i <= N; i++) {
    ll val_old = sgt_p.prod(0, i - 1);
    ll val_new = sgt_p.prod(0, i);
    if (val_old < 0 && val_new < 0) {
      cnt += 1 - val_new;
      ll val = sgt_p.get(i - 1);
      sgt_p.set(i - 1, val + 1 - val_new);
    } else if (val_old > 0 && val_new > 0) {
      cnt += val_new + 1;
      ll val = sgt_p.get(i - 1);
      sgt_p.set(i - 1, val - (val_new + 1));
    } else if (val_new == 0) {
      if (val_old > 0) {
        cnt++;
        ll val = sgt_p.get(i - 1);
  ...
  cnt = 0;
  segtree<ll, seg::op, seg::e> sgt_n(A);
  if (sgt_n.get(0) >= 0) {
    cnt += sgt_n.get(0) + 1;
    sgt_n.set(0, -1);
  }
  for (int i = 2; i <= N; i++) {
    ll val_old = sgt_n.prod(0, i - 1);
    ll val_new = sgt_n.prod(0, i);
    if (val_old < 0 && val_new < 0) {
      cnt += 1 - val_new;
      ll val = sgt_n.get(i - 1);
      sgt_n.set(i - 1, val + 1 - val_new);
    } else if (val_old > 0 && val_new > 0) {
      cnt += val_new + 1;
      ll val = sgt_n.get(i - 1);
      sgt_n.set(i - 1, val - (val_new + 1));
    } else if (val_new == 0) {
      if (val_old > 0) {
        cnt++;
        ll val = sgt_n.get(i - 1);
