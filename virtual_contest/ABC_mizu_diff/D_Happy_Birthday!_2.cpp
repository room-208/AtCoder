...
vector<int> GetB(vector<vector<vector<tuple<int, int, int>>>> path, int pi,
                 int pj) {
  vector<int> B;
  tuple<int, int, int> q = {pi, 1, pj};
  while (path[get<0>(q)][get<1>(q)][get<2>(q)] != make_tuple(-1, -1, -1)) {
    q = path[get<0>(q)][get<1>(q)][get<2>(q)];
    B.push_back(get<0>(q) + 1);
  }
  sort(B.begin(), B.end());
  return B;
}

vector<int> GetC(tuple<int, int, int> t,
                 vector<vector<vector<tuple<int, int, int>>>> path, int pi,
                 int pj) {
  vector<int> C;
  tuple<int, int, int> q = t;
  C.push_back(get<0>(q) + 1);
  while (path[get<0>(q)][get<1>(q)][get<2>(q)] != make_tuple(-1, -1, -1)) {
    q = path[get<0>(q)][get<1>(q)][get<2>(q)];
    C.push_back(get<0>(q) + 1);
  }
...
int main() {
  ...
  vector<vector<vector<bool>>> dp(
      N + 1, vector<vector<bool>>(2, vector<bool>(200, false)));
  vector<vector<vector<tuple<int, int, int>>>> path(
      N + 1, vector<vector<tuple<int, int, int>>>(
                 2, vector<tuple<int, int, int>>(200, {-1, -1, -1})));
  bool flag = false;
  tuple<int, int, int> t;
  int pi, pj;
  dp[0][0][0] = true;
  ...
  vector<int> B = GetB(path, pi, pj);
  vector<int> C = GetC(t, path, pi, pj);
