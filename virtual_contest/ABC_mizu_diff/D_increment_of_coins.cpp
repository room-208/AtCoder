...
int main() {
  ...
  for (int a = A; a < 100; a++) {
    for (int b = B; b < 100; b++) {
      for (int c = C; c < 100; c++) {
        double sum = a + b + c;
        double pa = ((double)a) / sum;
        double pb = ((double)b) / sum;
        double pc = ((double)c) / sum;
        dp[a + 1][b][c] += pa * dp[a][b][c];
        dp[a][b + 1][c] += pb * dp[a][b][c];
        dp[a][b][c + 1] += pc * dp[a][b][c];
      }
    }
  }
