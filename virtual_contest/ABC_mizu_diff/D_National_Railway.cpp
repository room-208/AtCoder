...
vector<vector<ll>> AccumMin2D(vector<vector<ll>> A) {
  int H = A.size();
  int W = A[0].size();
  vector<vector<ll>> S(H + 1, vector<ll>(W + 1, INF_ll));
  for (int i = 1; i <= H; i++) {
    for (int j = 1; j <= W; j++) {
      chmin(S[i][j], A[i - 1][j - 1]);
      chmin(S[i][j], S[i - 1][j]);
      chmin(S[i][j], S[i][j - 1]);
    }
  }
...
ll GetPattern1Val(vector<vector<ll>> A, ll C) {
  int H = A.size();
  int W = A[0].size();
  vector<vector<ll>> dp_ij(H, vector<ll>(W));
  vector<vector<ll>> dp_kl(H, vector<ll>(W));
  for (int i = 0; i < H; i++) {
    for (int j = 0; j < W; j++) {
      dp_ij[i][j] = A[i][j] - C * i - C * j;
    }
  }
  for (int k = 0; k < H; k++) {
    for (int l = 0; l < W; l++) {
      dp_kl[k][l] = A[k][l] + C * k + C * l;
    }
  }

  auto S_min = AccumMin2D(dp_ij);

  ll res = INF_ll;
  for (int k = 0; k < H; k++) {
    for (int l = 0; l < W; l++) {
      chmin(res, dp_kl[k][l] + S_min[k][l]);
      chmin(res, dp_kl[k][l] + S_min[k + 1][l]);
      chmin(res, dp_kl[k][l] + S_min[k][l + 1]);
    }
  }
  return res;
}

ll GetPattern2Val(vector<vector<ll>> A, ll C) {
  for (size_t i = 0; i < A.size(); i++) {
    reverse(A[i].begin(), A[i].end());
  }
  return GetPattern1Val(A, C);
}
...
int main() {
  ...
  ll ans = INF_ll;
  chmin(ans, GetPattern1Val(A, C));
  chmin(ans, GetPattern2Val(A, C));
