...
int main() {
  ...
  vector<ll> keys = {w[0], w[0] + 1, w[0] + 2, w[0] + 3};

  map<ll, vector<ll>> mp;
  for (auto &&key : keys) {
    mp[key].push_back(-INF_ll);
  }
  for (int i = 0; i < N; i++) {
  ...
  for (int k = 0; k < 4; k++) {
    vector<vector<ll>> dp_w(N + 1, vector<ll>(101, -1));
    for (int i = 0; i <= N; i++) {
      dp_w[i][0] = 0;
    }
    if (mp.count(keys[k])) {
      for (int i = 1; i <= N; i++) {
        ...
        if (i < mp[keys[k]].size()) {
          for (int j = 0; j < 100; j++) {
            chmax(dp_w[i][j + 1], dp_w[i - 1][j] + mp[keys[k]][i]);
          }
        }
      }
    }
    ...
    for (ll j = 0; j <= 100; j++) {
      ...
      for (ll k = 0; k <= 100; k++) {
        ...
        for (ll l = 0; l <= 100; l++) {
          ...
          if (i * keys[0] + j * keys[1] + k * keys[2] + l * keys[3] <= W) {
            chmax(ans, dp[0][N][i] + dp[1][N][j] + dp[2][N][k] + dp[3][N][l]);
          }
        }
      }
    }
