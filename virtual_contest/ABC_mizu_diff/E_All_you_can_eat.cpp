...
int main() {
  ...
  vector<vector<pair<ll, ll>>> dp(N + 1,
                                  vector<pair<ll, ll>>(T, {-INF_int, 0}));
  dp[0][0] = {0, 0};
  for (int i = 1; i <= N; i++) {
    for (int t = 0; t < T; t++) {
      if (dp[i - 1][t].first != -INF_int) {
        // 採用しない場合
        dp[i][t] = {dp[i - 1][t].first, max(dp[i - 1][t].second, B[i])};
      }
    }
    for (int t = 0; t < T; t++) {
      if (dp[i - 1][t].first != -INF_int) {
        // 採用する場合
        if (t + A[i] < T) {
          ll new_sum = dp[i - 1][t].first + B[i] + dp[i - 1][t].second;
          ll old_sum = dp[i][t + A[i]].first + dp[i][t + A[i]].second;
          if (new_sum > old_sum) {
            dp[i][t + A[i]] = {dp[i - 1][t].first + B[i], dp[i - 1][t].second};
          }
        }
      }
    }
  ...
  ll ans = -INF_int;
  for (int t = 0; t < T; t++) {
    ll sum = dp[N][t].first + dp[N][t].second;
    chmax(ans, sum);
  }
