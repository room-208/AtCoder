...
void dfs(const vector<vector<pair<int, int>>> &A, int &ans, vector<int> a = {},
         int depth = 0) {
  if (depth == A.size()) {
    int s = INF_int;
    for (int i = 0; i < a.size(); i++) {
  ...
  for (int i = 0; i < A[depth].size(); i++) {
    vector<int> a_new = a;
    if (A[depth][i].first != INF_int) {
      a_new.push_back(A[depth][i].first);
    }
    if (A[depth][i].second != INF_int) {
      a_new.push_back(A[depth][i].second);
    }
    dfs(A, ans, a_new, depth + 1);
  }
}
...
int main() {
  ...
  for (int i = 0; i < N; i++) {
    if (mp[D[i]] >= 3) {
      ...
      A[i].push_back({D[i], 24 - D[i]});
    } else {
      A[i].push_back({D[i], INF_int});
      A[i].push_back({24 - D[i], INF_int});
    }
  }
  A[N].push_back({0, INF_int});
  A[N].push_back({24, INF_int});
